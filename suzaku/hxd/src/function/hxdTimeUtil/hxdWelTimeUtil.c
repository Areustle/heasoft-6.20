/* ------------------------------------------------------------------------
 *   wel_time_determination
 *                         Time Determination function for Well data.
 * ------------------------------------------------------------------------*/
...
static void
debug_dump(unsigned int *WPU_time_counter, 
	   HxdTimeStatus *tstatus,
	   HxdTime *ACU_zero_time, HxdTime *tlm_time, HxdTime *well_time);

void hxdTimeUtil_modify_eventdata( double time, int mti, 
...
void
hxdTime_wel_time_determination(unsigned int WPU_time_counter, 
			       HxdTimeStatus *tstatus,
...
/*  unsigned int WPU_zero_time;*/ /* WPU_counter zero_time of wel_event_time */
	...
	WPU_clock_full_time = HXD_WPU_CLK_NORMAL_FULL_TIME_COUNT;
	WPU_shift = HXD_TIME_COUNTER_122_SHIFT; /* 122 us counter */
	shift = HXD_WPU_TIME_FULL_BIT_NUMBER + HXD_TIME_COUNTER_122_SHIFT;
	mask = HXD_WPU_CLK_NORMAL_MASK;
	break;
    case HXD_WPU_CLOCK_RATE_FINE:
    case HXD_WPU_CLOCK_RATE_NORMAL_OLD:
	WPU_clock_full_time = HXD_WPU_CLK_FINE_FULL_TIME_COUNT;
	WPU_shift = HXD_TIME_COUNTER_061_SHIFT; /* 61.0us counter */
	shift = HXD_WPU_TIME_FULL_BIT_NUMBER + HXD_TIME_COUNTER_061_SHIFT;
	mask = HXD_WPU_CLK_FINE_MASK;
	break;
    case HXD_WPU_CLOCK_RATE_SUPER_FINE:
    case HXD_WPU_CLOCK_RATE_SUPER_FINE2:
    case HXD_WPU_CLOCK_RATE_FINE_OLD:
	WPU_clock_full_time = HXD_WPU_CLK_S_FINE_FULL_TIME_COUNT;
	WPU_shift = HXD_TIME_COUNTER_030_SHIFT; /* 30.5us counter */
	shift = HXD_WPU_TIME_FULL_BIT_NUMBER + HXD_TIME_COUNTER_030_SHIFT;
	mask = HXD_WPU_CLK_S_FINE_MASK;
	break;
    case HXD_WPU_CLOCK_RATE_SUPER_FINE_OLD:
    case HXD_WPU_CLOCK_RATE_SUPER_FINE2_OLD:
	WPU_clock_full_time = HXD_WPU_CLK_SS_FINE_FULL_TIME_COUNT;
	WPU_shift = HXD_TIME_COUNTER_015_SHIFT; /* 15.7us counter */
	shift = HXD_WPU_TIME_FULL_BIT_NUMBER + HXD_TIME_COUNTER_015_SHIFT;
	mask = HXD_WPU_CLK_SS_FINE_MASK;
	break;
    default:
	WPU_clock_full_time = HXD_WPU_CLK_NORMAL_FULL_TIME_COUNT;
	WPU_shift = HXD_TIME_COUNTER_061_SHIFT; /* default = fine */
	shift = HXD_WPU_TIME_FULL_BIT_NUMBER + HXD_TIME_COUNTER_061_SHIFT;
	mask = HXD_WPU_CLK_FINE_MASK;
	break;
    }
...
/*	||  ! HxdTime_compare( tstatus->before.tlm_time[board], tlm_time ) */
	...
	WPU_time_counter_TLM  = ( (dummy.lower&mask) >> WPU_shift ) & 0x7ffff;
    ...
    well_time->upper += ((WPU_clock_full_time>>17)*(WPU_clock_number[board])) >>14;
    ...
    if (TIME_DEBUG) debug_dump(&WPU_time_counter, tstatus, &ACU_zero_time,
			       &tlm_time, well_time);
