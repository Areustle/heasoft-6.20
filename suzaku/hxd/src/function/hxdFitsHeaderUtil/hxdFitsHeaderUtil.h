#ifndef _HXD_FITS_HEADER_UTIL_H_
#define _HXD_FITS_HEADER_UTIL_H_
...
#define HXD_FITS_HEADER_UTIL_HDUCLASS_LC      5
...
void hxdFitsHeader_setDefaultKeywordValues(HXD_STD_KEYS *);

int hxdFitsHeader_updateStdTimeKeys(fitsfile*, HXD_STD_KEYS, int hdunum,
				    int *status );

int hxdFitsHeader_writeHXDStdKeys (fitsfile*, HXD_STD_KEYS, int hdunum,
				   int *status);

int hxdFitsHeader_timeUpdate (HXD_STD_KEYS*, double aetime);

void hxdFitsHeader_writeCredits(fitsfile * fp, char * task_name,
				char * task_version, char * credits, 
				int write_mode, int *status);

int hxdFitsHeader_writeTIMEDEL(fitsfile * fp, int extension_id, 
			       double timedel_val);

int hxdFitsHeader_readTIME(fitsfile* fp,
			   double* tstart, double* tstop,
			   double* telapse, double* ontime);

int hxdFitsHeader_writeParamer_wel(fitsfile *fp, int extension_id);
int hxdFitsHeader_writeParamer_scl(fitsfile *fp, int extension_id);
int hxdFitsHeader_writeParamer_trn(fitsfile *fp, int extension_id);
int hxdFitsHeader_writeParamer_bst(fitsfile *fp, int extension_id);
int hxdFitsHeader_writeParamer_arf(fitsfile *fp, int extension_id);
