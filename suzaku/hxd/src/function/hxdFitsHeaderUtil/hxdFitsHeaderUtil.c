 ...
 *         for hxdpi v2.4.1 (gsogpt)  V2.1.0      Y.Terada 2009-10-21
 *         for hxdpi v2.4.2 (gsogpt)  V2.1.1      Y.Terada 2010-01-05
 *         for hxdpi v2.4.2 (gsogpt)  V2.1.2      Y.Terada 2010-01-12
 *         bug fix, segmentation error            Y.Terada 2010-01-13
 *         add better light curve support
 *         add CHANMIN/CHANMAX keywords
 *         replace fits_write_key with fits_update_key
 *                          by C.Padgett 2011-03-24
 *         delete warnings                        Y.Terada 2013-10-16
 */
...
static char *pname = "hxdFitsHeaderUtil";

static HXD_FITS_HEADER_CREDITS_BUF credits_buf;

#define FILELIST_MAX_LENGTH PIL_LINESIZE

static int hxdFitsHeader_extruct_credits( char* credits);

/* STD HDU header keyword */
void hxdFitsHeader_setDefaultKeywordValues(HXD_STD_KEYS *stdkeys)
{
...
int hxdFitsHeader_writeHXDStdKeys (fitsfile * fp, HXD_STD_KEYS stdkeys,
...
/*char comment[73], hduclas1[10]; */
  ...
  if (fits_update_key_str(fp, "TELESCOP", stdkeys.telescope, 
			 "Telescope (mission) name", status)) {
    fprintf(stderr,"Error in writing keyword: TELESCOP (status=%d)\n",*status);
    return ANL_NG;
  }
  if (fits_update_key_str(fp, "INSTRUME", stdkeys.instrument, 
  ...
  if (stdkeys.use_detnam) {
    if (fits_update_key_str(fp, "DETNAM", stdkeys.detnam, 
  ...
  if (fits_update_key_str(fp, "OBS_MODE", stdkeys.obs_mode, 
  ...
  if (stdkeys.hduclass_ev_hk != HXD_FITS_HEADER_UTIL_HDUCLASS_HK){
    if (fits_update_key_str(fp, "DATAMODE", stdkeys.datamode, 
			   "Data mode", status)) {
  ...
  if (stdkeys.write_radec == 0) {
    ...
    if (fits_update_key_dbl(fp, "RA_OBJ", stdkeys.ra_obj,6,
			   "Planned target R.A. (deg)", status)) {
      fprintf(stderr,"Error in writing keyword: RA_OBJ (status=%d)\n",*status);
      return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "DEC_OBJ", stdkeys.dec_obj,6,
			   "Planned target DEC. (deg)", status)){
      fprintf(stderr,"Error in writing keyword: DEC_OBJ (status=%d)\n",*status);
      return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "RA_PNT", stdkeys.ra_pnt,6,
			    "average optical axis location R.A. (deg)", 
			    status)) {
      fprintf(stderr,"Error in writing keyword: RA_PNT (status=%d)\n",*status);
      return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "DEC_PNT",stdkeys.dec_pnt,6,
			    "average optical axis location DEC. (deg)", 
			    status)) {
      fprintf(stderr,"Error in writing keyword: DEC_PNT (status=%d)\n",*status);
      return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "RA_NOM",stdkeys.ra_nom,6,
			    "Nominal satellite pointing direction R.A. (deg)", 
			    status)) {
      fprintf(stderr,"Error in writing keyword: RA_NOM (status=%d)\n",*status);
      return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "DEC_NOM",stdkeys.dec_nom,6,
			    "Nominal satellite pointing direction DEC. (deg)", 
			    status)){
      fprintf(stderr,"Error in writing keyword: DEC_NOM (status=%d)\n",*status);
      return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "PA_NOM",stdkeys.roll_nom,6,
			    "Nominal position angle from North to DETY (deg)", 
			    status)){
      fprintf(stderr,"Error in writing keyword: PA_NOM (status=%d)\n",*status);
      return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "MEAN_EA1",stdkeys.mean_ea1,6,
			    "Mean of the 1st ZYZ-Euler angle (deg)", 
			    status)){
      fprintf(stderr,"Error in writing keyword: MEAN_EA1 (status=%d)\n",*status);
      return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "MEAN_EA2",stdkeys.mean_ea2,6,
			    "Mean of the 2nd ZYZ-Euler angle (deg)", 
			    status)){
    fprintf(stderr,"Error in writing keyword: MEAN_EA2 (status=%d)\n",*status);
    return ANL_NG;
    }
    if (fits_update_key_dbl(fp, "MEAN_EA3",stdkeys.mean_ea3,6,
			   "Mean of the 3ed ZYZ-Euler angle (deg)", 
			   status)){
      fprintf(stderr,"Error in writing keyword: MEAN_EA3 (status=%d)\n",*status);
      return ANL_NG;
  ...
  if (fits_update_key_fixdbl(fp, "MJDREFF",   stdkeys.mjd_reff, 17, 
			    "fractional part of the MJD reference (64.184 s)",
			    status)){
    fprintf(stderr,"Error in writing keyword: MJDREFF (status=%d)\n",*status);
    return ANL_NG;
  }
  if (fits_update_key_str(fp, "TIMEREF",  stdkeys.timeref, 
  ...
  if (fits_update_key_str(fp, "TIMEUNIT", stdkeys.timeunit, 
			 "unit for time related keywords", status)){
    fprintf(stderr,"Error in writing keyword: TIMEUNIT (status=%d)\n",*status);
    return ANL_NG;
  }
  if (fits_update_key_str(fp, "TASSIGN",  stdkeys.tassign, 
			 "TASSIGN", status)) {
    fprintf(stderr,"Error in writing keyword: TASSIGN (status=%d)\n",*status);
    return ANL_NG;
  }
  if (fits_update_key_log(fp, "CLOCKAPP", stdkeys.clockapp, 
			 "CLOCKAPP", status)){
    fprintf(stderr,"Error in writing keyword: OBJECT (status=%d)\n",*status);
    return ANL_NG;
  }
  if (stdkeys.hduclass_ev_hk != HXD_FITS_HEADER_UTIL_HDUCLASS_HK){
    if (fits_update_key_null(fp, "TIMEDEL",
    ...
    if (fits_update_key_flt(fp, "TIMEPIXR", stdkeys.timepixr, 6, 
    ...
    if (fits_update_key_flt(fp, "TIERRELA", stdkeys.tierrela, 6, 
			   "short-term clock stability", status)){
      fprintf(stderr,"Error in writing keyword: TIERRELA (status=%d)\n",
	      *status);
      return ANL_NG;
    }
    if (fits_update_key_flt(fp, "TIERABSO", stdkeys.tierabso, 6, 
    ...
    /*
    if (fits_update_key_null(fp, "TIERRELA","short-term clock stability",
			    status)) {
      fprintf(stderr,"Error in writing keyword: TIERRELA (status=%d)\n",
	      *status);
      return ANL_NG;
    ...
    if (fits_update_key_null(fp, "TIERABSO","absolute precision of the clock",
			    status)) {
  ...
  /* write CHANMIN/MAX/TYPE keywords for light curves */
  if (stdkeys.hduclass_ev_hk == HXD_FITS_HEADER_UTIL_HDUCLASS_LC){
    if (stdkeys.chanmin >= 0 && stdkeys.chanmax >= 0 ) {
      if (fits_update_key_lng(fp, "CHANMIN", stdkeys.chanmin,
                              "Low energy channel", status)){
        fprintf(stderr,"Error in writing keyword: CHANMIN (status=%d)\n",
                *status);
        return ANL_NG;
      }
      if (fits_update_key_lng(fp, "CHANMAX", stdkeys.chanmax,
                              "High energy channel", status)){
        fprintf(stderr,"Error in writing keyword: CHANMAX (status=%d)\n",
                *status);
        return ANL_NG;
      }
      if (fits_update_key_str(fp, "CHANTYPE", "PHA",
                              "Energy channel type", status)){
        fprintf(stderr,"Error in writing keyword: CHANTYPE (status=%d)\n",
                *status);
        return ANL_NG;
      }
    }
  ...
  if (fits_update_key_str(fp, "HDUCLASS", stdkeys.hduclass, 
  ...
  if (stdkeys.hduclass_ev_hk == HXD_FITS_HEADER_UTIL_HDUCLASS_EVENT){
    if (fits_update_key_str(fp, "HDUCLAS1", "EVENTS",
    ...
    if (fits_update_key_str(fp, "HDUCLAS2", "ALL", 
    ...
    if (fits_update_key_str(fp, "HDUCLAS1", "TEMPORALDATA",
			   "type of data (e.g. EVENTS/TEMPORALDATA)", 
			   status)){
      fprintf(stderr,"Error in writing keyword: HDUCLAS1 (status=%d)\n",
	      *status);
    ...
    if (fits_update_key_str(fp, "HDUCLAS2", "HKP", "housekeeping parameters",
			   status)){
      fprintf(stderr,"Error in writing keyword: HDUCLAS2 (status=%d)\n",
	      *status);
      return ANL_NG;
    ...
    if (fits_update_key_str(fp, "HDUCLAS1", "SPECTRUM",
    ...
    if (fits_update_key_str(fp, "HDUVERS1", "1.2.0",
    ...
    if (fits_update_key_str(fp, "HDUVERS", "1.2.0", 
    ...
    if (fits_update_key_str(fp, "HDUCLAS3", "COUNT", 
    ...
    if (fits_update_key_str(fp, "HDUCLAS1", "IMAGE",
    ...
    if (fits_update_key_str(fp, "HDUNAME", "WMAP", 
			   "HDU Name", status)){
    ...
    if (fits_update_key_str(fp, "HDUCLAS2", "WMAP", 
    ...
    if (fits_update_key_str(fp, "CONTENT", "SPECTRUM", 
  ...
  } else if (stdkeys.hduclass_ev_hk == HXD_FITS_HEADER_UTIL_HDUCLASS_LC){
    if (fits_update_key_str(fp, "HDUCLAS1", "LIGHTCURVE",
                            "Extension contains a light curve",
			    status)){
      fprintf(stderr,"Error in writing keyword: HDUCLAS1 (status=%d)\n",
	      *status);
      return ANL_NG;
    }
    if (fits_update_key_str(fp, "HDUCLAS2", "TOTAL",
                            "Extension contains source + background",
			    status)){
      fprintf(stderr,"Error in writing keyword: HDUCLAS1 (status=%d)\n",
	      *status);
      return ANL_NG;
    }
    if (fits_update_key_str(fp, "HDUCLAS3", "RATE",
                            "Extension contains count/time interval data",
			    status)){
      fprintf(stderr,"Error in writing keyword: HDUCLAS1 (status=%d)\n",
	      *status);
      return ANL_NG;
    }
    if (fits_update_key_str(fp, "HDUVERS1", "1.1.0",
		            "Version number of the format", status)){
        fprintf(stderr,"Error in writing keyword: HDUVERS1 (status=%d)\n",
                *status);
        return ANL_NG;
    }
  } else {
    if (fits_update_key_null(fp, "HDUCLAS1",
			    "type of data (e.g. EVENTS/TEMPORALDATA)",
			    status)) {
      fprintf(stderr,"Error in writing keyword: HDUCLAS1 (status=%d)\n",
	      *status);
    ...
    if (fits_update_key_null(fp, "HDUCLAS2",
  ...
  if (fits_update_key_str(fp, "CREATOR",  stdkeys.creator, 
			 "by HXD team",status)){
    fprintf(stderr,"Error in writing keyword: CREATOR (status=%d)\n",*status);
    return ANL_NG;
  }
  
  if (fits_update_key_str(fp, "ORIGIN",   stdkeys.origin, 
  ...
  if(*status){
    fprintf(stderr,"hxdFitsComment_write_single ('CREATOR') failed (status=%d)\n",
	    *status);
    return ANL_NG;
  }

  hxdFitsHeader_writeCredits(fp, stdkeys.task_name, stdkeys.task_version,
			     stdkeys.credits, 
			     HXD_FITS_HEADER_UTIL_CREDIT_CREATE,
			     status);
  if(*status){
    fprintf(stderr,"hxdFitsHeader_writeCredits failed (%d)\n",
	    *status);
    return ANL_NG;
  }
...
void
hxdFitsHeader_writeCredits(fitsfile * fp, char * task_name, 
  ...
  anl_status = hxdFitsHeader_extruct_credits(credits);
...
int hxdFitsHeader_updateStdTimeKeys(fitsfile *fp, HXD_STD_KEYS stdkeys,
				    int hdunum, int *status) 
{
  int hdutype;
  char comment[] = "&";
...
int hxdFitsHeader_timeUpdate (HXD_STD_KEYS *stdkeys, double aetime)
{
...
/******* static function ************/
static int
hxdFitsHeader_extruct_credits( char* credits){
...
int 
hxdFitsHeader_writeTIMEDEL(fitsfile *fp, int extension_id, double timedel_val){
...
void hxdFitsHeader_mallocSTDKEYS(HXD_STD_KEYS *stdkeys){
...
void hxdFitsHeader_freeSTDKEYS(HXD_STD_KEYS *stdkeys){
...
int 
hxdFitsHeader_readTIME(fitsfile* fp,
...
int 
hxdFitsHeader_writeParamer_wel(fitsfile *fp, int extension_id){
  int stat = HXD_FITS_HEADER_UTIL_OK;
  int istat = 0;
  int size;
  ...
  if(hxdpi_yn){
    ...
    /** ------------------ gso_gainhist_name  --------------------- **/
    size = -1;
    BnkGet("HXD:PIL:hxd_gsogpt_fname", 
	   sizeof(char)*HXDFITSHEADER_LINESIZE,
    ...
    size = -1;
    BnkGet("HXD:PIL:CALDB_TYPE:gsogpt",sizeof(int), &size, 
	   &piltype_caldb_gsoghf);
    if(piltype_caldb_gsoghf){
      sprintf(history, "      hxd_gsogpt_fname = %s (CALDB)", 
	      gso_gainhist_name);
    } else {
      sprintf(history, "      hxd_gsogpt_fname = %s", gso_gainhist_name);
    }
    fits_write_history(fp, history, &istat);
    if(istat) {
      fprintf(stderr, 
    ...
    /** ------------------ GSOGPT keywords --------------------- **/
    BnkGet("HXDpi:GSO_GPT_NAME", sizeof(hxd_gsogpt_fname),
	   &size, hxd_gsogpt_fname);
    BnkGet("HXDpi:GSOGPT_VERSION", sizeof(int), 
	   &size, &gso_gainhist_ver);

    stat = hxdFitsHeader_addGSOGPT_wel(fp, extension_id, 
				       hxd_gsogpt_fname, gso_gainhist_ver);
    if(stat == HXD_FITS_HEADER_UTIL_NG){
      fprintf(stderr, "%s:hxdFitsHeader_writeParamer_wel failed(GSOGPT_V)\n",
	      pname);
      stat = HXD_FITS_HEADER_UTIL_NG;
      return stat;
    }
  ...
  if (hxdpi_yn) {
    fits_update_key_log(fp, "GSOOLDPI", 0,
...
int hxdFitsHeader_addGSOGPT_wel(fitsfile *welfp, int extension_id,
				char * gsogpt_f, int gsogpt_v){
  int stat = HXD_FITS_HEADER_UTIL_OK;
  int istat = 0;
  int hdutype;
  ...
  /** get the basename of the gsogpt file **/
  gsogpt_f_base = aefits_basename( gsogpt_f );

  /** (1) init **/
  if (fits_movabs_hdu(welfp, extension_id, &hdutype, &istat)) {
    fprintf(stderr, "%s: fits_movabs_hdu failed (status=%d)\n",
	    pname, istat);
    stat = HXD_FITS_HEADER_UTIL_NG;
    return stat;
  ...
  /** (2) Search keyword **/
  for(i=1;;i++){
    if (fits_flush_file(welfp, &istat)){
      fprintf(stderr, "Error doing fits_flush_file() (stat=%d)\n", istat);
    }
    
    if (fits_read_record(welfp, i, tmp_card, &istat)) {
      istat = 0; break;
    }

    if(0 == strncmp("GSOGPT_F", tmp_card, 8)){
      found = 1;
      break;
    }
  }

  /** (3) Insert keywords if no exist**/
  if (! found) {
    for(i=1;;i++){
      if (fits_flush_file(welfp, &istat)){
	...
	istat = 0; break;
      ...
      /* --- Insert keys before HXD_FVER --- */
      if(0 == strncmp("HXD_FVER", tmp_card, 8)){
	fits_insert_record(welfp, i, "GSOGPT_F", &istat);
	if(istat){
	  fprintf(stderr, "Error in writing GSOGPT_F (%d)\n", istat);
	  break;
	}
	
	fits_insert_record(welfp, i, "GSOGPT_V", &istat);
	if(istat){
	  fprintf(stderr, "Error in writing GSOGPT_V (%d)\n", istat);
	  break;
	}
  ...
  /** (4) Update keywords **/
  if (fits_update_key_str(welfp, "GSOGPT_F", gsogpt_f_base, 
			  "Name of gsogpt used by hxdpi", 
			  &istat)) {
    fprintf(stderr,"Error in writing keyword: GSOGPT_F (%d)\n",
	    istat);
    return ANL_NG;
  }
  
  gpt_ver = (long) gsogpt_v;
  if (fits_update_key_lng(welfp, "GSOGPT_V", gpt_ver, 
			  "Version of gsogpt by hxdpi", 
			  &istat)) {
    fprintf(stderr,"Error in writing keyword: GSOGPT_V (%d)\n",
	    istat);
    return ANL_NG;
  }
...
int 
hxdFitsHeader_writeParamer_scl(fitsfile *fp, int extension_id){
  int stat = HXD_FITS_HEADER_UTIL_OK;
  int istat = 0;
  int size;
  char history     [HXDFITSHEADER_LINESIZE];
...
int 
hxdFitsHeader_writeParamer_trn(fitsfile *fp, int extension_id){
  int stat = HXD_FITS_HEADER_UTIL_OK;
  int istat = 0;
  int size;
  char history     [HXDFITSHEADER_LINESIZE];
...
int 
hxdFitsHeader_writeParamer_bst(fitsfile *fp, int extension_id){
  int stat = HXD_FITS_HEADER_UTIL_OK;
  int istat = 0;
  int size;
  char history     [HXDFITSHEADER_LINESIZE];
...
int 
hxdFitsHeader_writeParamer_arf(fitsfile *fp, int extension_id){
  int stat = HXD_FITS_HEADER_UTIL_OK;
  int istat = 0;
  /* int size; */
  /* char history     [HXDFITSHEADER_LINESIZE]; */
