...
char HXDpi_old_version[] = "version 2.1.3";
static char pname[] = "HXDpi";

void
HXDpi_old_startup(int *status){ 
  int used = 1;
...
void	
HXDpi_old_his(int *status){ *status = ANL_OK; }

void
HXDpi_old_bgnrun(int *status){
  ...
  BnkfGetM("HXDpi:GSO_GHT_NAME", sizeof(hxd_gsoght_fname),
	   &size, hxd_gsoght_fname);
  ...
  hxdpiUtil_old_gso_init(hxd_gsoght_fname, hxd_gsolin_fname, 
		         tstart, tstop,  &istat);
  ...
  hxdpiUtil_old_pin_init(hxd_pinghf_fname, hxd_pinlin_fname, &istat);
  if(istat != HXDPIUTIL_OK){
...
void
HXDpi_old_ana(int nevent, int eventid, int *status){
  int size;
  int pi_slow, pi_fast;
  ...
  hxdpiUtil_old_HK  hk_data;
  hxdpiUtil_old_EHK ehk_data;
  int temp;
  ...
  if(event_data.time == HXD_TIME_INVALID){
    ...
    upi_pin3 = HXD_UPI_INVALID;  /* INVARID CH DATA */
      ...
      BnkfGetM("HXDpi:HKDATA",  sizeof(hxdpiUtil_old_HK),  &size, &hk_data);
      BnkfGetM("HXDpi:EHKDATA", sizeof(hxdpiUtil_old_EHK), &size, &ehk_data);
      hxdpiUtil_old_pin_correct(&event_data,
			    &upi_pin0,&upi_pin1,&upi_pin2,&upi_pin3,
			    &pi_pin0,&pi_pin1,&pi_pin2,&pi_pin3);
      hxdpiUtil_old_gso_correct(&event_data, &hk_data, &ehk_data,
		                &upi_fast,&upi_slow, &pi_fast,&pi_slow);
  }

  hxdpiUtil_old_modify_eventdata(upi_fast, upi_slow, upi_pin0, upi_pin1,
			     upi_pin2, upi_pin3, 
...
void	
HXDpi_old_endrun(int *status){ *status = ANL_OK; }

void
HXDpi_old_exit(int *status){ *status = ANL_OK; }
