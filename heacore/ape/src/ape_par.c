...
/*
 * $Log: ape_par.c,v $
 * Revision 1.109  2015/09/27 19:47:54  peachey
 * 1. Only check for correct values of "mode" parameter when value checking
 *    is enabled. Trim some white spaces from the source code. (ape_par).
 * 2. Revise/add unit test for bad "mode" parameter to test more specifically
 *    for the case of "a" (invalid for "mode" parameter only) and other
 *    invalid "mode" parameter values, like "qh".
 * 3. Check for validity of parameter file while opening. Do not check
 *    values though, so that bad values may be fixed by the user through
 *    running the calling code that uses the parameter file. (ape_session).
 * 4. Update expected unix reference unit test log to match the above
 *    changes.
 *
