...
vector<Integer> rmf::RandomChannels(const vector<Real>& energy, const vector<Integer>& NumberPhotons)
{
  Integer GratingOrder(-999);
...
vector<Integer> rmf::RandomChannels(const Real energy, const Integer NumberPhotons, const Integer GratingOrder)
{
  ...
  return this->RandomChannels(energyArray, NPhotArray, GratingOrder);
}

// Return vector of randomly generated channel numbers for a set of energies and a
// grating order. Use GratingOrder = -999 as special case to ignore grating information

vector<Integer> rmf::RandomChannels(const vector<Real>& energy, const vector<Integer>& NumberPhotons, const Integer GratingOrder)
{
  Integer NumberOut(0);
  ...
  for (size_t i=0; i<energy.size(); i++) {
	...
	energybin = lower;
      ...
      vector<Real> RandomNumber(NumberPhotons[i]);
      unsigned long int seed = (unsigned long int)time(NULL);
      HDmtInit(seed);
      for (size_t j=0; j<(size_t)RandomNumber.size(); j++) RandomNumber[j] = (Real) HDmtDrand();
      HDmtFree();

      // loop round the photons

      for (size_t j=0; j<(size_t)RandomNumber.size(); j++) {
	...
	if ( RandomNumber[j] <= sumresponse[upper] ) {
	  while ( upper - lower > 1 ) {
	    middle = (upper + lower)/2;
	    if ( RandomNumber[j] < sumresponse[middle] ) {
	      upper = middle;
	    } else {
	      lower = middle;
	    }
	  }
	  if ( RandomNumber[j] > sumresponse[lower] ) {
	    channel[iout] = upper;
	  } else {
	    channel[iout] = lower;
	  }
