...
Fast Fourier/Cosine/Sine Transform
    ...
    table       :use
functions
    scdft2d: Complex Discrete Fourier Transform
    srdft2d: Real Discrete Fourier Transform
    sddct2d: Discrete Cosine Transform
    sddst2d: Discrete Sine Transform
function prototypes
...
#include <stdio.h>
#include <stdlib.h>
#define sfft2d_alloc_error_check(p) { \
    if ((p) == NULL) { \
...
#define sfft2d_thread_create(thp,func,argp) { \
    if (pthread_create(thp, NULL, func, (void *) (argp)) != 0) { \
        fprintf(stderr, "fft2d thread error\n"); \
        exit(1); \
    } \
}
#define sfft2d_thread_wait(th) { \
...
void scdft2d(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void smakewt(int nw, int *ip, float *w);
    void scdft(int n, int isgn, float *a, int *ip, float *w);
    void scdft2d_sub(int n1, int n2, int isgn, float **a, float *t, 
        int *ip, float *w);
#ifdef USE_FFT2D_THREADS
    void sxdft2d0_subth(int n1, int n2, int icr, int isgn, float **a, 
        int *ip, float *w);
    void scdft2d_subth(int n1, int n2, int isgn, float **a, float *t, 
        int *ip, float *w);
    ...
    if (t == NULL) {
        ...
        sfft2d_alloc_error_check(t);
    }
#ifdef USE_FFT2D_THREADS
    if ((float) n1 * n2 >= (float) FFT2D_THREADS_BEGIN_N) {
        sxdft2d0_subth(n1, n2, 0, isgn, a, ip, w);
        scdft2d_subth(n1, n2, isgn, a, t, ip, w);
    } else 
#endif /* USE_FFT2D_THREADS */
    {
        for (i = 0; i < n1; i++) {
            scdft(n2, isgn, a[i], ip, w);
        }
        scdft2d_sub(n1, n2, isgn, a, t, ip, w);
    }
    if (itnull != 0) {
        free(t);
    }
...
void srdft2d(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void smakewt(int nw, int *ip, float *w);
    ...
    void scdft2d_sub(int n1, int n2, int isgn, float **a, float *t, 
        int *ip, float *w);
    void srdft2d_sub(int n1, int n2, int isgn, float **a);
#ifdef USE_FFT2D_THREADS
    void sxdft2d0_subth(int n1, int n2, int icr, int isgn, float **a, 
        int *ip, float *w);
    void scdft2d_subth(int n1, int n2, int isgn, float **a, float *t, 
    ...
    if ((float) n1 * n2 >= (float) FFT2D_THREADS_BEGIN_N) {
        if (isgn < 0) {
            srdft2d_sub(n1, n2, isgn, a);
            scdft2d_subth(n1, n2, isgn, a, t, ip, w);
        }
        sxdft2d0_subth(n1, n2, 1, isgn, a, ip, w);
        if (isgn >= 0) {
            scdft2d_subth(n1, n2, isgn, a, t, ip, w);
            srdft2d_sub(n1, n2, isgn, a);
        }
...
void srdft2dsort(int n1, int n2, int isgn, float **a)
{
    int n1h, i;
    float x, y;
...
void sddct2d(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void smakewt(int nw, int *ip, float *w);
    void smakect(int nc, int *ip, float *c);
    void sddct(int n, int isgn, float *a, int *ip, float *w);
    void sddxt2d_sub(int n1, int n2, int ics, int isgn, float **a, 
        float *t, int *ip, float *w);
#ifdef USE_FFT2D_THREADS
    void sddxt2d0_subth(int n1, int n2, int ics, int isgn, float **a, 
        int *ip, float *w);
    void sddxt2d_subth(int n1, int n2, int ics, int isgn, float **a, 
        float *t, int *ip, float *w);
#endif /* USE_FFT2D_THREADS */
    int n, nw, nc, itnull, nthread, nt, i;
...
#ifdef USE_FFT2D_THREADS
    if ((float) n1 * n2 >= (float) FFT2D_THREADS_BEGIN_N) {
        sddxt2d0_subth(n1, n2, 0, isgn, a, ip, w);
        sddxt2d_subth(n1, n2, 0, isgn, a, t, ip, w);
    } else 
#endif /* USE_FFT2D_THREADS */
    {
        for (i = 0; i < n1; i++) {
            sddct(n2, isgn, a[i], ip, w);
        }
        sddxt2d_sub(n1, n2, 0, isgn, a, t, ip, w);
    }
    if (itnull != 0) {
        free(t);
    }
...
void sddst2d(int n1, int n2, int isgn, float **a, float *t, 
    int *ip, float *w)
{
    void smakewt(int nw, int *ip, float *w);
    void smakect(int nc, int *ip, float *c);
...
#ifdef USE_FFT2D_THREADS
    if ((float) n1 * n2 >= (float) FFT2D_THREADS_BEGIN_N) {
        sddxt2d0_subth(n1, n2, 1, isgn, a, ip, w);
        sddxt2d_subth(n1, n2, 1, isgn, a, t, ip, w);
    } else 
#endif /* USE_FFT2D_THREADS */
    {
        for (i = 0; i < n1; i++) {
            sddst(n2, isgn, a[i], ip, w);
        }
        sddxt2d_sub(n1, n2, 1, isgn, a, t, ip, w);
    }
    if (itnull != 0) {
        free(t);
    }
    ...
    int *ip, float *w)
{
    ...
    if (n2 > 4) {
        for (j = 0; j < n2; j += 8) {
            ...
            scdft(2 * n1, isgn, t, ip, w);
            scdft(2 * n1, isgn, &t[2 * n1], ip, w);
            scdft(2 * n1, isgn, &t[4 * n1], ip, w);
            scdft(2 * n1, isgn, &t[6 * n1], ip, w);
            for (i = 0; i < n1; i++) {
                a[i][j] = t[2 * i];
                a[i][j + 1] = t[2 * i + 1];
                a[i][j + 2] = t[2 * n1 + 2 * i];
    ...
    float *t, int *ip, float *w)
{
    ...
    if (n2 > 2) {
        for (j = 0; j < n2; j += 4) {
            ...
            if (ics == 0) {
                sddct(n1, isgn, t, ip, w);
                sddct(n1, isgn, &t[n1], ip, w);
                sddct(n1, isgn, &t[2 * n1], ip, w);
                sddct(n1, isgn, &t[3 * n1], ip, w);
            } else {
                sddst(n1, isgn, t, ip, w);
                sddst(n1, isgn, &t[n1], ip, w);
                sddst(n1, isgn, &t[2 * n1], ip, w);
                sddst(n1, isgn, &t[3 * n1], ip, w);
            }
            for (i = 0; i < n1; i++) {
                a[i][j] = t[i];
                a[i][j + 1] = t[n1 + i];
    ...
    int *ip, float *w)
{
    ...
    for (i = 0; i < nthread; i++) {
        ...
        ag[i].ip = ip;
        ag[i].w = w;
        sfft2d_thread_create(&th[i], sxdft2d0_th, &ag[i]);
    }
    for (i = 0; i < nthread; i++) {
        sfft2d_thread_wait(th[i]);
    }
}
    ...
    int *ip, float *w)
{
    ...
    for (i = 0; i < nthread; i++) {
        ...
        ag[i].ip = ip;
        ag[i].w = w;
        sfft2d_thread_create(&th[i], scdft2d_th, &ag[i]);
    }
    for (i = 0; i < nthread; i++) {
        sfft2d_thread_wait(th[i]);
    }
    ...
    int *ip, float *w)
{
    ...
    for (i = 0; i < nthread; i++) {
        ...
        ag[i].ip = ip;
        ag[i].w = w;
        sfft2d_thread_create(&th[i], sddxt2d0_th, &ag[i]);
    }
    for (i = 0; i < nthread; i++) {
        sfft2d_thread_wait(th[i]);
    }
    ...
    float *t, int *ip, float *w)
{
    ...
    for (i = 0; i < nthread; i++) {
        ...
        ag[i].ip = ip;
        ag[i].w = w;
        sfft2d_thread_create(&th[i], sddxt2d_th, &ag[i]);
    }
    for (i = 0; i < nthread; i++) {
        sfft2d_thread_wait(th[i]);
    }
