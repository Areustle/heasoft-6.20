 ...
 * $Id: batmask.h,v 1.27 2005/09/23 04:33:46 craigm Exp $
 *
...
struct batmaskcorrections_struct {
  ...
  int *detmask;      /* Detector mask (1=good; 0=bad) */
  int dmx, dmy;      /* Detector mask X and Y dimensions (or 0 for none) */
  ...
  int cfast;         /* Computate with faster but lower fidelity code (1/0=Y/N)*/
  int cnearfield;    /* Compute nearfield correction 1=yes; 0=no */

  int ngoodpix;      /* Upon return: Number of good detectors in weight computation */
  double pcodefr;    /* Upon return: Partial coding fraction in weight computation */
  double maskwtsqf;  /* Upon return: Mask weight squared factor */
  int cmaskwt;       /* Correct for mask weight technique? 1=yes; 0=no */
  double maskwtswgain;/* Mask weighting software gain factor;
			 maskwtnorm should be multiplied by (1+maskwtswgain) */
  int cbadcosine;    /* Apply incorrect cosine correction? 1=yes; 0=no */
};
...
/* Structure defining detector tops (J. Cumming) */
...
extern int maskwtimg_op(double srcpos[3], 
		   struct batmaskplane_struct *mask, 
		   struct batdetplane_struct *detplane,
		   struct batmaskcorrections_struct *corrs,
		   double *maskimg, int nimgx, int nimgy);
