...
*/
void	makeback0(picstruct *field, picstruct *wfield)
  {
...
/*
  This is the hook for installing a user provided background mean and sigma.
*/
void	makeback(picstruct *field, picstruct *wfield)
  {
   if (field->back_type == BACK_MEAN_SIGMA)
     {
      if (prefs.nback_val > 1)
        {
         NFPRINTF(OUTPUT, "Using user provided background mean/sigma");
         field->backmean = (float) prefs.back_val[0];
         field->backsig = (float) prefs.back_val[1];
        }
      else
        error(EXIT_FAILURE,
          "*Error*: two values [mean,sigma] required for BACK_TYPE MEAN_SIGMA",
          "");
     }
   else
     makeback0(field, wfield);
