/*
 *	nu_termio.c: --- terminal input/output module ---
 *
 *	DESCRIPTION:
 ...
 *      1.1.7 - NS 25/10/2010 - Modified to be included in NuSTAR libraries
 *      1.1.8 - NS 31/08/2012 - Added CreateAbsSymbolicLink routine
 *      1.1.9 - NS 06/11/2012 - Added RenameFile routine
 *	AUTHOR	:
 *		UL, Thu May 5 16:10:55 METDST 1994
 */

#define NU_TERMIO_C
#define NU_TERMIO_VERSION	1.1.9
...
#include <unistd.h>  /* needed by 'symlink' */
#include <limits.h>  /* needed by 'realpath' */
#include <stdlib.h>  /* needed by 'realpath' */
#include <errno.h>   /* needed to manage return of 'rename' */
...
int RenameFile(char *source, char *destination)
{
  ...
  if (rename(source, destination) == -1)
    {
      /* source and destination are not on the same mounted filesystem. */
      if( errno==EXDEV )
	{
...
int CreateAbsSymbolicLink(char *filename, char *linkname)
{
  char resolved_path[PATH_MAX];

  if( !realpath(filename,resolved_path) )
    {
      Error("CreateAbsSymbolicLink", "Could not resolve absolute path for '%s'", filename);
      return NOT_OK;
    }

  if( symlink(resolved_path, linkname) )
    {
      Error("CreateAbsSymbolicLink", "Could not create symbolic link '%s' to '%s' ", linkname, resolved_path);
      return NOT_OK;
    }
