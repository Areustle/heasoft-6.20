/*
 *	nu_highfits.c: set of high-level interface routines to the CFITSIO library
 *
 *	DESCRIPTION:
 *
 *	CHANGE HISTORY:
 *		NS  25/10/10: 0.1.0: - working version from XRTDAS
 *		NS  14/12/11: 0.1.1: - Added 'CopyHDUExt' routine
 *		NS  14/03/12: 0.1.2: - Added 'GetHDUExtNum' routine
 *
 *     
 *
 *                                     
...
#define NU_HIGHFITS_C
#define NU_HIGHFITS_VERSION		0.1.2
...
int CopyHDUExt(char *infile, char *outfile, char *extname){

  fitsfile *inunit=NULL, *outunit=NULL;   /* Input and Output fits file pointer */
  int     status=OK;
  char    taskname[MAXFNAME_LEN];
  ...
  /* Open input file */
  if ( fits_open_file(&inunit, infile, READONLY, &status) )
    {
  ...
  /* Open output file */
  if ( fits_open_file(&outunit, outfile, READWRITE, &status) )
    {
      headas_chat(NORMAL,"%s: Error: Unable to open\n", taskname);
      headas_chat(NORMAL,"%s: Error: '%s' file.\n", taskname, outfile);
      fits_report_error(stdout, status);
  ...
  /* Copy the HDU from input to output file */
  if(fits_copy_hdu(inunit, outunit, 0, &status))
    {
      headas_chat(NORMAL, "%s: Error: Unable to copy HDU from '%s' input file to\n", taskname, infile);
      headas_chat(NORMAL, "%s: Error: to '%s' output file.\n", taskname, outfile);
      goto CopyHDUExt_end;
    }
...
int GetHDUExtNum(char *infile, char *extname, int *extnum){

  fitsfile *inunit=NULL;   /* Input fits file pointer */
  int     status=OK;
  char    taskname[MAXFNAME_LEN];
  ...
  /* Get ext number */
  if (!fits_get_hdu_num(inunit, extnum))
    {
