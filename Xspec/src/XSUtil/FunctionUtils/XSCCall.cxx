                         ...
                         RealArray& fluxArray, RealArray& fluxErrArray, const string& initString) const
{
        ...
        XSutility::auto_array_ptr<Real> pEnergy(convert(energyArray));
        XSutility::auto_array_ptr<Real> pParam(convert(params));
        Real* energy  = pEnergy.get();
        Real* param   = pParam.get();

        int nEar = nArray - 1; 

        XSutility::auto_array_ptr<Real> pFlux(new Real[nEar]);
        XSutility::auto_array_ptr<Real> pFluxErr(new Real[nEar]);
        Real* flux = pFlux.get();
...
template <> 
void XSCall<MathExpression>::operator() (const RealArray& energyArray, const RealArray& param, int spectrumNumber,
                         RealArray& fluxArray, RealArray& fluxErrArray, const string& initString) const
{
