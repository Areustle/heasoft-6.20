...
void FNINIT()
{
   ...
   // Initialize the directory for the data files.  Use the
   // XSPEC_MDATA_DIR environment variable if it is set, otherwise
   // look in the manager directory.
   const char* cModelDataLoc = getenv("XSPEC_MDATA_DIR");
   ...
   spectralLoc = string(cHeadasLoc) + "/../spectral/";
   ...
   if (cModelDataLoc)
   {
      modelDataLoc = string(cModelDataLoc);
   }
   else
   {
   ...
   managerLoc = spectralLoc + "manager";
   ...
   FunctionUtility::neiVersion("3.0.4");
}
