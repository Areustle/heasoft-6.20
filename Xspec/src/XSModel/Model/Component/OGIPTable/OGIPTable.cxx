...
void OGIPTable::getInterpolantIndices ()
{
   ...
   std::vector<IntegerArray> prepRecordNumbers(Ninter);
...
void OGIPTable::energyWeights (const UniqueEnergy* uniqueEng)
{
  ...
  IntegerArray& startBin = m_startWeightBin[uniqueEng];
  IntegerArray& endBin   = m_endWeightBin[uniqueEng];        
  ...
  else
  {
     if (outputBin > 0) 
     {
        ...
        oss << "\n No model information is available for observed frame energies below "
            << std::setw(14) << observedEnergy[0] << " so model is set to zero at these energies\n";
...
void OGIPTable::clearArrays (const std::set<UniqueEnergy*>& currentEngs)
{
  ...
  std::map<const UniqueEnergy*,IntegerArray>::iterator m (m_startWeightBin.begin());
  std::map<const UniqueEnergy*,IntegerArray>::iterator mEnd (m_startWeightBin.end());
