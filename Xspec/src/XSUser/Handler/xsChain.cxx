...
{
  enum SUBCOMS {BURN=1, CLEAR, FILETYPE, INFO, LENGTH, LOAD, PROPOSAL, 
		RAND, RECALC, RESCALE, RUN, STAT, TEMPERATURE, TYPE, UNLOAD, WALKERS};
  ...
  if (nArgs == 1 || (subcom = rawArgs[1]) == "?") {
    ...
    const size_t width = varPars.size() + 1;
    try {
      size_t tmpInput=0;
      string tmpMsg;
      ...
      case INFO:
	{
	  ...
	  if (!nChains) {
	    ...
	    map<int,ModParam*>::const_iterator ivp = varPars.begin();
	    map<int,ModParam*>::const_iterator ivpEnd = varPars.end();
	    size_t ip(0);
	    while (ivp != ivpEnd) {
	      tcout << "        " << ivp->second->getParameterLabel() 
		    << ".  " << parVals[ip] << endl;
	      ivp++;
	      ip++;
	    }

	    tcout << "Loaded chains:   length:" << endl;
	...
	if (nArgs < 3) {
	  ...
	  std::auto_ptr<Chain> apChain(new Chain(apIO.get(),thirdArg));
	...
	if (nArgs < 3 && choice != RECALC) {
	  ...
	  if (choice == RECALC) {
		...
		tcerr << "***Error: To use " << propName << " proposal, must also specify "
		      << "\nfit | chain | diagonal | matrix | <filename>" << std::endl;
		return -1;
	      }
	      const string sourceQualifier(XSutility::lowerCase(fourthArg));
	      // sourceQualifier may be an abbreviation for fit|chain|diagonal|matrix.
      ...
      case RESCALE:
	{
	  ...
	  if (!(ssThirdArg >> factor) || !ssThirdArg.eof()) {
		...
		for (size_t j=0; j<=i; j++) {
	      ...
	      chainManager->setChainProposal(propParts[0], initString);
	    }
	  }
	}
	break;
	...
	if (nArgs < 3) {
	  ...
	  if (isAppend) {
	    ...
	    // If it exists, must prompt user.
	    if (chainManager->getChain(fileName)) {
	...
	if (nArgs > 2) {
	  string modName;
	  size_t iPar=0;
	  if ((XSparse::stringIntPair(thirdArg, modName, iPar)
	       && iPar > 0)|| iPar > 0) {
	  ...
	  // parSpec will always have proper string:int syntax,
	  // no need to check.
	  string modName;
	  size_t iPar=0;
	  XSparse::stringIntPair(parSpec, modName, iPar);
	  chainManager->calcStat(iPar, modName);
	}
	break;
      case TEMPERATURE:
