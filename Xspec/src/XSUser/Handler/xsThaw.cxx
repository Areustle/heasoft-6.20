...
int
XSGlobal::xsThaw(ClientData cdata,Tcl_Interp* tclInterp,int objc, Tcl_Obj* CONST  objv[])
{
   StringArray rawArgs;
   HandlerUtils::tclArgsToCpp(objc, objv, rawArgs);
   int status = doThaw(rawArgs);
   if (!status)
...
int
XSGlobal::doThaw(const StringArray& rawArgs)
{
   ...
   const bool isRespPar = (rawArgs[0] == string("rthaw"));
   const char* cmd = isRespPar ? "rthaw" : "thaw";
   try
   {
      string defName;
      StringArray inArgs;
      ...
      while (itModGroups != itModGroupsEnd)
      {
         ...
         if (isRespPar)
            isChanged = freezeThawUntie<ResponseParam,ThawStrategy>
                              (itModGroups->first,itModGroups->second, prevRange);
         else
            isChanged = freezeThawUntie<Parameter,ThawStrategy>
                              (itModGroups->first,itModGroups->second, prevRange);
         if (isChanged)
            isAnyChanged = true;
...
namespace
{
   bool ThawStrategy::perform(Parameter* par, string& msg)
   {
      bool isPerformed = false;
      if (!par->isFrozen())
      {
         msg = string(" is not frozen.");
      }
      else
      {
         // Unlike freeze, don't enclose in try/catch.  If something
         // throws below, it is a more serious problem and should
         // therefore break out of loops.  If parameter is not a
         // ModParam this simply does nothing.
         par->thaw();
         isPerformed = true;
      }
      return isPerformed;
