...
;proc highlight {w str args} {
    ...
    set args [::Utility::get_opts opts $args \
	    {-nocase 0 -regexp 0 -tag 1 -color 1}]
...
widget create Console -type frame -base text -components {
    ...
    {scrollbar yscrollbar sy {-takefocus 0 -bd 2 -width 10 \
...
;proc construct {w} {
    ...
    grid $data(yscrollbar) $data(console)  -sticky news
    grid columnconfig $w 0 -weight 1
    grid rowconfig $w 1 -weight 1

#    prompt $w "console display active\n"
    prompt $w "Welcome to XSPEC++\n"
#    puts stderr [info globals pow*]
...
#	$w insert end [EvalSlave subst $data(-prompt)] prompt
	$w insert end [list XSPEC [history nextid]>] prompt
    }
    $w mark set output $i
...
;proc _error {W} {
    ...
    pack $w.sy -side $data(scrollypos) -fill y
