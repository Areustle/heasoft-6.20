...
// read from files for the requested elements

int IonBalNei::Read(vector<int> Z, string dirname)
{
  int status = 0;
  ...
  for (size_t i=0; i<Z.size(); i++) {
    string filename = dirname + "eigen" + atomName[Z[i]-1] + versionname + ".fits";
    int read = ReadElement(Z[i], filename);
    std::ostringstream oss;
    if ( read != 0 ) {
      oss << "Cannot read eigenvector data for Z=" << Z[i] 
	  << " from filename " << filename << "\n";
      xs_write(const_cast<char*>(oss.str().c_str()),15);
    } else {
      oss << "Reading eigenvector data for Z=" << Z[i]
	  << " from filename " << filename << "\n";
...
int IonBalNei::Read(int Z, string dirname)
{
  vector<int> Zarray(1);
  Zarray[0] = Z;
  int status = Read(Zarray, dirname);
  return status;
}

// read from file

int IonBalNei::ReadElement(int Z, string filename)
{

  const string hduName("EIGEN");

  const vector<string> hduKeys;
  const vector<string> primaryKeys;

  // open file
  auto_ptr<FITS> pIonBalfile(0);

  try {
    pIonBalfile.reset(new FITS(filename,CCfits::Read,hduName,false,hduKeys,primaryKeys,(int)1));
  } catch(...) {
    return(1);
  }

  ExtHDU& ionbalExt = pIonBalfile->currentExtension();
  ...
  if ( NumberTemperatures != NUMBTEMP ) return(2);
...
void calcNEIfractions(const Real& Te, const Real& tau, const IntegerArray& Z, vector<RealArray>& IonFrac)
{
...
void calcNEIfractions(const RealArray& Te, const RealArray& tau, 
		      const IntegerArray& Z, const vector<RealArray>& initIonFrac,
...
void calcNEIfractions(const RealArray& Te, const RealArray& tau, 
		      const RealArray& weight, const IntegerArray& Z, vector<RealArray>& IonFrac)
{
  vector<RealArray> initIonFrac(0);
  calcNEIfractions(Te, tau, weight, Z, initIonFrac, IonFrac);
  return; 
...
void calcNEIfractions(const RealArray& Te, const RealArray& tau, 
		      const RealArray& weight, const IntegerArray& Zarray, 
		      ...
		      vector<RealArray>& IonFrac)
{
  ...
  static IntegerArray saveZarray;
  ...
  // fully ionized
    ...
    if ( Z == 1 ) {
      ...
      // check to see whether data for the requested Z has been read in 
      // and if not do so
	...
	dataAvailable = false;
	if ( fileShouldExist ) {
	  int read = NeiData.Read(Z, datadir);
	  if ( read == 0 ) dataAvailable = true;
	}
      }
...
void calcCEIfractions(const Real Te, const IntegerArray& Z, vector<RealArray>& IonFrac)
{
  Real tau(-1.0);
...
string writeIonFrac(const IntegerArray& Z, const vector<RealArray>& IonFrac)
{
  std::ostringstream oss;
  oss << "Ion Fractions:\n";
...
// be in utility routines. Better would be to set up RealArray and IntegerArray
