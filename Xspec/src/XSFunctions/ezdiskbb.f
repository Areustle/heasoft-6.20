...
c                  array is not calculated by this program.
     ...
     +            F1, F2, F, FNEW(0:24999), ENEW(0:24999), Z
      ...
      character(256) datdir, filenm1, filenm2
...
c      file "ezdiskbbf.dat," with the corresponding energy bins in "energies.dat."
c      Th data from these files is read by the code once, and the values for 
c      the fluxes and energies are then saved in the variables
c      "flux" and "e," respectively.  The value of "iread" is then set to 99
c       so that the data files will not be read again. 
...
c this model does not calculate errors
      ...
      if (iread .ne. 99) then
         ...
         datdir = fgmodf()
         filenm1 = datdir(:lenact(datdir))//'ezdiskbbe.dat'
         filenm2 = datdir(:lenact(datdir))//'ezdiskbbf.dat'
         CALL xwrite (' opening and reading ezdiskbb data files ', 10)

         CALL getlun(ilun)
         open (ilun, file=filenm1, form='formatted', status='old')
         read(ilun,*) (e(i),i=0,24999)
             close (ilun)
                
         open (ilun, file=filenm2, form='formatted', status='old')
         read(ilun,*) (flux(i),i=0,24999)
             close (ilun)
         CALL frelun(ilun)
