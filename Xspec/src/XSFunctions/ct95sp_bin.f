...
c                          tloalflo
c                          storall
c            ctcompspec    compt       zgammi       zgserr      zgamln
c                                                   zfcff       zgamln
c                                      zgamln
c                                      zyyit2       zgamln
c                          d00         zgamln
...
C********************************************************************
      SUBROUTINE ZGSERR(GAMSER,A,X,GLN)
      implicit none
...
C SERIES REPRESENTATION OF INCOMPLETE GAMMA FUNCTION
      ...
      double precision ap, sum, del, zgamln
      integer n
      GLN = ZGAMLN(A)
      IF (X.LE.0.0) THEN
         IF (X.LT.0.) PAUSE
...
C***********************************************************
      SUBROUTINE ZGCFF(GAMMCF,A,X,GLN)
      implicit double precision(a-h,o-z)
...
c     write(*,*) 'gcff: a1 = ',a1
 ...
 11   CONTINUE
      PAUSE 'A too large ITMAX too small'
 20   gammcf=exp(-x+a*dlog(x)-gln)*g
      return
      end
