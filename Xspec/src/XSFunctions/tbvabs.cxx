        ...
        const string& initString)
{
   ...
   if ( version.substr(0,1) == "1" ) {
     ...
     auto_array_ptr<float> apSiggas(new float[ne]);
     auto_array_ptr<float> apSiggrains(new float[ne]);
     auto_array_ptr<float> apSigmol(new float[ne]);
     auto_array_ptr<float> apSigavg(new float[ne]);
     ...
     XSFunctions::stlToFloatArrays<double>(energyArray, params, flux, fluxErr,
					  ear, pars, photar, photer);
     auto_array_ptr<double> apEar(ear);
     auto_array_ptr<double> apPars(pars);
     auto_array_ptr<double> apPhotar(photar);
     auto_array_ptr<double> apPhoter(photer);
