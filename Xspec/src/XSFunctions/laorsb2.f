      ...
      SUBROUTINE laorsb2(ear, ne, param, nenrgy, nrad, nteta, efil, 
...
C Subroutine to do the actual calculation of the LAOR2 model
C Parameters :
...
c      print *,'ir0,rad(ir0)',ir0,rad(ir0)
       if ( r .lt. rad(ir0) ) THEN
          ems=area*r**(-param(2))
       ELSEIF ( r .eq. rad(ir0) ) THEN
          ems=(param(6)-rad(ir0+1))/(r-rad(ir0+1))
     &      *area*r**(-param(2))
     &      + (1.-(param(6)-rad(ir0+1))/(r-rad(ir0+1)))
     &      * area*param(6)**(-param(2)+param(7))*r**(-param(7))
       ELSE
          ems=area*param(6)**(-param(2)+param(7))
     &       *r**(-param(7))
       ENDIF
