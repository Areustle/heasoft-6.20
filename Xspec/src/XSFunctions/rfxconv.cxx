		  ...
		  const RealArray& energyArray, RealArray& flux)
{
  ...
  for (int i=0; i<Ne; ++i) X[i] = (energyArray[i+1]+energyArray[i]) * factor;
  ...
  if ( Scale != 0.0 ) {
    ...
    if ( factor != factor ) {
      tcout << "NaN detected : " << factor << " " << Sprefp[j14] << " " << Spref[j14] 
		  ...
		  RealArray& tableParams, Real index, RealArray& Spref)
{
  ...
  if ( tableParams[1] < 1.4 ) {
    tcout << modelName << ": A 2-10 keV slope of " << tableParams[1] << " is outside the tabulated range - setting to 1.4" << endl;
    tableParams[1] = 1.4;
  } else if ( tableParams[1] > 3.3 ) {
    tcout << modelName << ": A 2-10 keV slope of " << tableParams[1] << " is outside the tabulated range - setting to 3.3" << endl;
    tableParams[1] = 3.3;
  }
  ...
  int ifl = 0;
  additiveTable(energyArray, tableParams, FileName, ifl, Spref, fluxErrArray, " ");
  if ( Spref.size() == 0 ) tcout << modelName << ": Failed to read " << FileName << endl;
