...
// Set the noLines, noResonanceLines, thermalBroadening and velocityBroadening 
// checking the APECNOLINES, APECNORES, APECTHERMAL and APECVELOCITY values
...
// and APECMINFLUX values and set the object
			...
			const Real velocity, const Real flux)
{
  ...
  if ( noLines ) {
	...
	oss << "Thermal or velocity broadening will be applied only for lines with flux > " << minimumLinefluxForBroadening << std::endl;
      }
    }
  }
...
int Aped::ReadTemperature(const vector<int>& TemperatureIndex)
{
  ...
  for (size_t iTemp=0; iTemp<TemperatureIndex.size(); iTemp++) {
	...
	return(5);
      ...
      IntegerArray Element(NumberLines);
      IntegerArray Ion(NumberLines);
      IntegerArray ElementDriver(NumberLines);
      IntegerArray IonDriver(NumberLines);
      IntegerArray UpperLev(NumberLines);
      IntegerArray LowerLev(NumberLines);
	...
	try {
      ...
      IntegerArray Z(NumberRows);
      IntegerArray rmJ(NumberRows);
      IntegerArray Ncont(NumberRows);
      IntegerArray Npseudo(NumberRows);
...
void Aped::SumEqSpectra(const RealArray& energyArray, 
			const IntegerArray& Zinput, const RealArray& abunZ,
		      ...
		      RealArray& fluxArray, RealArray& fluxErrArray)
{
  ...
  for (size_t it=0; it<Tinput.size(); it++) {
    ...
    Real tdelta = tval[itemp[1]] - tval[itemp[0]];
    Real tcoeff[2];
    if ( tdelta >  0.0 ) {
    ...
    std::ostringstream oss;
    oss  << "Interpolating between " << tval[itemp[0]]
  ...
  for (size_t it=0; it<interpTindex.size(); it++) {
	...
	if ( coeff > 0.0 ) {
	  ...
	  apedInterpFlux(thisIon.ContinuumEnergy, thisIon.ContinuumFlux, 
			 coeff, sourceFrameEnergy, fluxArray);
  ...
  for (size_t it=0; it<interpTbindex.size(); it++) {
	...
	if ( haveLines && !noLines && acoeff > 0.0 ) {
	  ...
	  if ( lastLine == -1 ) lastLine = nLines-1;
	  ...
	  for (size_t i=0; i<nLinesNeeded; i++) {
	    ...
	    if ( lineflux[i] >= minimumLinefluxForBroadening ) {
	  ...
	  calcManyLines(sourceFrameEnergy, energy, width, lineflux, 
			(Real)1.0e-6, 0, qspeedy, fluxArray);
  ...
  FunctionUtility::loadDbValue("ApecMaxLineflux", maxLineflux);
		    ...
		    RealArray& fluxArray)
{
  ...
  size_t lastE = inputEnergy.size() - 1;

  Real eMin = inputEnergy[0];
  Real eMax = inputEnergy[lastE];

  size_t ihigh = 0;

  for (size_t ien=0; ien<energyArray.size()-1; ien++) {

    Real energy = (energyArray[ien] + energyArray[ien+1])/2.0;

    if ( energy >= eMin && energy <= eMax ) {

      while ( energy > inputEnergy[ihigh] && ihigh <= lastE ) ihigh++;
      size_t ilow = ihigh - 1;

      fluxArray[ien] += coeff *
	( (inputEnergy[ihigh]-energy)*inputFlux[ilow]
	  +(energy-inputEnergy[ilow])*inputFlux[ihigh] )
	* ( energyArray[ien+1] - energyArray[ien] )
	/ (inputEnergy[ihigh]-inputEnergy[ilow]);
		    ...
		    RealArray& fluxArray, RealArray& fluxErrArray)
{
  ...
  static string saveVersion = FunctionUtility::atomdbVersion();
  ...
  if ( isFirst || version != saveVersion ) {
    ...
    string contfil = rootfil + "_coco.fits";
    string linefil = rootfil + "_line.fits";
    std::ifstream test1(contfil.c_str());
    if (!test1) {
      rootfil = datadir + version;
      contfil = rootfil + "_coco.fits";
      linefil = rootfil + "_line.fits";
  ...
  // APECNOLINES and APECNORES xset parameters

  AtomicData.SetLineSpecs(noLines, false, qtherm, velocity, 0.0);
		   ...
		   RealArray& fluxArray, RealArray& fluxErrArray)
{
  ...
  AtomicData.SetLineSpecs(false, false, qtherm, velocity, 0.0);
...
int KBcalcNEISpectrum(const RealArray& energyArray, const IntegerArray& Zinput,
		    ...
		    RealArray& fluxArray, RealArray& fluxErrArray)
{
  static bool isFirst(true);
  static Aped AtomicData;
  static string saveEigenVersion = FunctionUtility::neiVersion();
  static string saveNEIapecroot = FunctionUtility::atomdbVersion();
  string contfil, linefil;

  string pname("NEIVERS");
  string eigenVersion(FunctionUtility::getModelString(pname));
  if (!eigenVersion.length() || eigenVersion == FunctionUtility::NOT_A_KEY())
    eigenVersion = saveEigenVersion;

  // Check for a valid eigenVersion string

  if ( eigenVersion != "1.0" && eigenVersion != "1.1" && 
       eigenVersion != "2.0" && eigenVersion.substr(0,2) != "3." ) {
    std::ostringstream oss;
    oss << eigenVersion << " is not a valid NEIVERS, ignoring." << "\n";
    FunctionUtility::xsWrite(oss.str(),10);
    eigenVersion = saveEigenVersion;
  }
  ...
  // Check for NEIAPECROOT having been set

  pname = "NEIAPECROOT";
  string NEIapecroot(FunctionUtility::getModelString(pname));
  if (!NEIapecroot.length() || NEIapecroot == FunctionUtility::NOT_A_KEY())
    NEIapecroot = saveNEIapecroot;


  if ( isFirst || eigenVersion != saveEigenVersion || 
       NEIapecroot != saveNEIapecroot ) {
    saveEigenVersion = eigenVersion;
    saveNEIapecroot = NEIapecroot;

    // set the names for the AtomDB files. If NEIapecroot has been set then
    // try a number of possibilities - a version number, a root filename,
    // a complete directory path and a complete directory path including
    // the final _nei.

    const string& datadir = FunctionUtility::modelDataPath();
    if ( NEIapecroot != FunctionUtility::NOT_A_KEY()) {
      string rootfil = datadir + "apec_v" + NEIapecroot;
      contfil = rootfil + "_nei_comp.fits";
      linefil = rootfil + "_nei_line.fits";
      std::ifstream test1(contfil.c_str());
      if (!test1) {
	rootfil = datadir + NEIapecroot;
	contfil = rootfil + "_nei_comp.fits";
	linefil = rootfil + "_nei_line.fits";
	std::ifstream test2(contfil.c_str());
	if (!test2) {
	  contfil = NEIapecroot + "_nei_comp.fits";
	  linefil = NEIapecroot + "_nei_line.fits";
	  std::ifstream test3(contfil.c_str());
	  if (!test3) {
	  contfil = NEIapecroot + "_comp.fits";
	  linefil = NEIapecroot + "_line.fits";
	  }
	}
      }
    } else if ( eigenVersion == "2.0" ) {
      contfil = datadir + "APEC_nei_v11_comp.fits";
      linefil = datadir + "APEC_nei_v11_line.fits";
    } else if ( eigenVersion.substr(0,2) == "3." ) {
      contfil = datadir + "apec_v" + eigenVersion + "_nei_comp.fits";
      linefil = datadir + "apec_v" + eigenVersion + "_nei_line.fits";
    }

    std::ostringstream oss;
    oss << "Reading continuum data from " << contfil << "\n"
  ...
  // Check for values of APECTHERMAL, APECVELOCITY, APECMINFLUX, APECNOLINES 
  // and APECNORES xset parameters
