...
c     this is number of bins in my tables (between ~0.1 and 200 keV)
     ...
     $     sptemp(4, nr, nmaxp), eart(ne2), phottmp(ne2), enel
      ...
      INTEGER np, nread, ir, lnm, iener, len
...
c     and picks the grid
      ...
      if (icall.eq.0) then
         slovo = fgmodf()
         lenn = lenact(slovo)
         slovo = slovo(1:lenn)//'xspectr1.6'
         CALL getlun(ilun)
         open(ilun, status = 'old', file = slovo, iostat=ierr)
         IF ( ierr .NE. 0 ) THEN
...
c      parameter (ntest = 1441, ncmax = 16)      
     ...
     5     weight(ncmax), afe(3)
      ...
      REAL fxfda, ecut0, flun, tdm, cosv, airon, fxdtem, costem
      REAL afetemp, dgrav, dtot, sum, devtot, dfxd, diang, dabun
      REAL v1, v2, v3, v4, rinit
      INTEGER lenn, ilun, ierr, jh, m, icall, i, kr, np, ntmax
      INTEGER jhmin, jhmax, jmmin, jmmax, ifemin, ifemax, j
      INTEGER ncexc, nclose, io, jcount
...
c         mu_v(m) = (m-0.5)/(float(mumax_v))
 ...
 303  format (1pe9.1,1pe9.1,1pe10.2,1pe10.2,1pe11.3,I4)
      ...
      filenm = fgmodf()
      lenn = lenact(filenm)
      filenm = filenm(1:lenn)//'newtable12'      
      CALL getlun(ilun)
      open (ilun, status = 'old', file =filenm(:lenact(filenm)), 
     &      iostat=ierr)      

      kr = 0
      do while (kr.lt.2)

         read(ilun, 303, END = 120) abund(i), gam1(i), ecut1(i), 
     1        fxd1(i), tauex(i), iang(i)
         read(ilun,*, END = 120)  grav(i), flun, tdm
c     if (i.gt.1440) abund(i) = 5. * abund(i)
         iang(i) = iang(i)/2
         do np = 1, nmaxp-1
            read (ilun,*, END = 120) int_out(i, np+1)
         enddo
