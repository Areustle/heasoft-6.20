...
C INCOMPLETE GAMMA FUNCTION
      REAL A , X , GAMMI , gln , gammcf , gamser
...
C USE THE SERIES REPRESENTATION
      ...
      RETURN
      END
**==gserr.spg  processed by SPAG 4.50J  at 15:47 on 21 Sep 1995
C********************************************************************
      SUBROUTINE GSERR(Gamser,A,X,Gln)
      IMPLICIT NONE
C SERIES REPRESENTATION OF INCOMPLETE GAMMA FUNCTION
      REAL A , X , Gln , Gamser , ap , sum , del , EPS
      REAL GAMLN
      INTEGER ITMAX , n
...
c      write(*,*) 'gserr: a  x  logx',a,x,log(x)
 ...
 200  Gamser = sum*EXP(-X+A*LOG(X)-Gln)
      RETURN
      END
**==gcff.spg  processed by SPAG 4.50J  at 15:47 on 21 Sep 1995
C***********************************************************
      SUBROUTINE GCFF(Gammcf,A,X,Gln)
      IMPLICIT NONE
C CONTINUED FRACTION METHOD FOR INCOMPLETE GAMMA FUNCTION
      REAL a0 , b0 , a1 , b1 , Gln , EPS , Gammcf , A , X , anf , g , 
     &     gold , fac , ana , an
      INTEGER ITMAX , n
      REAL GAMLN
      character(255) contxt
...
c            write(*,*) 'gcff: a1 = ',a1
 ...
 100  CONTINUE
     ...
     & 'Warning: Inc. Gamma Fn. GCFF did not converge for A = ', A
      CALL xwrite(contxt, 10)
 200  Gammcf = EXP(-X+A*ALOG(X)-Gln)*g
      RETURN
      END
