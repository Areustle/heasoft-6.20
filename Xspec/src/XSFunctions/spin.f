...
c
      ...
      integer nradii,ng,ia,imu0,abins,mu0bins,ilun,ios
...
c
      ...
      mu0=cos(theta0)
c
c     Read in transfer function from the kerrtable.dat file.
c
      if ( qfirst ) then
         call getlun(ilun)
         datafile = fgmodf()
         datafile=datafile(1:lenact(datafile))//'/kerrtable.dat'
         call openwr(ilun,datafile,'old',' ',' ',0,0,ios)
         if ( ios .NE. 0 ) THEN
...
C          w = w(n): array of the weights.
      ...
      DOUBLE PRECISION PI,XM,XL,Z,P1,P2,P3,Z1,PP,EPS
