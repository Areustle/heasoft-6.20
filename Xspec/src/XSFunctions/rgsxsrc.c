...
#include "cfortran.h"
#include "FunctionUtils/xsFortran.h"
#include <math.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
*/
  ...
  {
    char s1[2048],s2[2048],s3[2048],s4[2048],s5[2048],s6[2048];
    char line[2048];
    char *parfile;
    int  nconv;
    FILE *fp;
	...
	while (fgets(line,2048,fp)) {
	  ...
	  switch (nconv) {
	  case 5:
	    sprintf(s6,"%s %s %s %s",s2,s3,s4,s5);
	    sprintf(s2,"%s",s6);
	    break;
	  case 4:
	    sprintf(s6,"%s %s %s",s2,s3,s4);
	    sprintf(s2,"%s",s6);
	    break;
	  case 3:
	    sprintf(s6,"%s %s",s2,s3);
	    sprintf(s2,"%s",s6);
	    break;
	  default:
	    break;
	  ...
	  case 5:	  case 4:	  case 3:	  case 2:
		...
		if ( strcmp(exs->image_val,s2) != 0 ) {
		  fprintf(stderr,"value in file %s supercedes previous value!!\n"
		          "setting variable %s to %s\n was %s\n",parfile,s1,s2,
                          exs->image_val);
                  exs->image_val = malloc(sizeof(s2));
                  strcpy(exs->image_val,s2);
	          exs->initialized=0;
                }
              } else {
                exs->image_val = malloc(sizeof(s2));
		...
		if ( strcmp(exs->bores_val,s2) != 0 ) {
		  fprintf(stderr,"value in file %s supercedes previous value!!\n"
		          "setting variable %s to %s\n was %s\n",parfile,s1,s2,
                          exs->bores_val);
                  exs->bores_val = malloc(sizeof(s2));
                  strcpy(exs->bores_val,s2);
	          exs->initialized=0;
                }
              } else {
                exs->bores_val = malloc(sizeof(s2));
		...
		if ( strcmp(exs->extraction_val,s2) != 0 ) {
		  fprintf(stderr,"value in file %s supercedes previous value!!\n"
		          "setting variable %s to %s\n was %s\n",parfile,s1,s2,
                          exs->extraction_val);
                  exs->extraction_val = malloc(sizeof(s2));
                  strcpy(exs->extraction_val,s2);
	          exs->initialized=0;
                }
              } else {
                exs->extraction_val = malloc(sizeof(s2));
  ...
  if (1) {
    ...
    for (i=0;i<ne;i++) {
      ...
      if (c1==NCONV) c1--;    if (c1) c1--; /* since it's numrec */
      if (c2==NCONV) c2--;    if (c2) c2--; /* since it's numrec */
    ...
    {
      float power,*conv_func,*conv,*kernel;
    ...
    for (j=0;j<NCONV;j++) {
      ...
      if (c1==ne+1) c1--;    if (c1) c1--; /* since it's numrec */
      if (c2==ne+1) c2--;    if (c2) c2--; /* since it's numrec */
