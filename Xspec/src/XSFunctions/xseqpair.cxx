...
void gauleg(const Real x1, const Real x2, RealArray& x, RealArray& w);
...
// routine to calculate the Gauss-Legendre integration coefficients.
// algorithm by Rybicki via Press et al.
...
void gauleg(const Real x1, const Real x2, RealArray& x, RealArray& w)
{
  ...
  for (int i=0; i<m; i++) {
    ...
    Real z = cos(3.141592654*(i+0.75)/(n+0.5));
    Real z1 = 0.0;
