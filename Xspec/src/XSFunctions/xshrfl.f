...
c this model does not calculate errors
      ...
      DO i = 1, ne
         e1=zfac*ear(i-1)
         e2=zfac*ear(i)
         a=hrefl(param(1),param(2),param(3),e1,param(4),
     1        param(5))
         b=hrefl(param(1),param(2),param(3),e2,param(4),
     1        param(5))
         photar(i) = 0.5*(2.0*param(6)+param(7)*(a+b))
      enddo
      return
      end
      REAL FUNCTION HREFL(D1,D2,TOBS,E,AFE,EKEDGE)
C
...
C
     ...
     1   F2
      ...
      ENDIF
      HREFL=ALBEDO*HMEAN(ALBEDO)*HFUNA(ALBEDO,XMU)*F2*0.5
      END
C
C -------
C
