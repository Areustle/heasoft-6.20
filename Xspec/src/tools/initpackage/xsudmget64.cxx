...
/* $Id: xsudmget64.cxx,v 1.1 2014/10/14 19:25:40 gordon Exp $

   Explanatory comments added July 1999 Ben Dorman indicated  by BD 7/99

   $Log: xsudmget64.cxx,v $
   Revision 1.1  2014/10/14 19:25:40  gordon
   File to provide new -udmget64 option when building with initpackage.  This
   duplicates the original xsudmget.cxx except that it changes the types of the
   pointer variables from int and unsigned int to long and unsigned long.

   This was driven by the Mac platform, where builds are now 64-bit by default.  64-bit Mac builds require 'long' to hold pointer values, AND require that the calling Fortran code declare the array pointer variables as INTEGER*8 rather than INTEGER*4.

   64-bit Linux users can continue using the old xsudmget.cxx with INTEGER*4.  However if they use this new xsudmget.cxx, they should also make the corresponding INTEGER*8 change in their code.
