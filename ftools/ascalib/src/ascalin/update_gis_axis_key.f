...
C SUBROUTINE:
C
C     update_gis_axis_key
C
C DESCRIPTION:
C     Update Event file GIS keywords following succesfull completion of 
C     processing.
C
C AUTHOR/DATE:
...
C
C USAGE:
C     call update_gis_axis_key(data_unit, cal_name, cal_time,
...
C******************************************************************************
        ...
        subroutine update_gis_axis_key(data_unit, cal_name, cal_time,
     ...
     &          ': CAL FILE: ', cal_name(1:134)
           call ftmkys (data_unit, 'GISLIN1', comment, ' ', status)
           if (status .eq. 202) then
              status = 0
              call ftpkls (data_unit, 'GISLIN1', comment, ' ', status)
           end if
           
           write(comment,'(a14, a23, a43)') taskname,
     &          ': CAL FILE DATE STAMP: ', cal_time(1:43)
           call ftmkys (data_unit, 'GISLIN2', comment, ' ', status)
           if (status .eq. 202) then
              status = 0
              call ftpkls (data_unit, 'GISLIN2', comment, ' ', status)
           end if
           
c           write(comment,'(a14, a13, a133)') taskname,
c     &          ': GAIN FILE: ', gain_name(1:133)
c           call ftmkys (data_unit, 'GISLIN3', comment, ' ', status)
c           if (status .eq. 202) then
c              status = 0
c              call ftpkls (data_unit, 'GISLIN3', comment, ' ', status)
c           end if
           
c           write(comment,'(a14, a24, a42)') taskname,
c     &          ': GAIN FILE DATE STAMP: ', gain_time(1:42)
c           call ftmkys (data_unit, 'GISLIN4', comment, ' ', status)
c           if (status .eq. 202) then
c              status = 0
c              call ftpkys (data_unit, 'GISLIN4', comment, ' ', status)
c           end if
     ...
     &       'Last  PI channel enumeration', status)
        ...
        call modifyn_j_kw(data_unit, 'TLMIN', rti_col, 0, 
     &       'First RTI channel enumeration', status)
        
        call modifyn_j_kw(data_unit, 'TLMAX', rti_col, rti_size-1,
     ...
     &       decimals, 'Detector X reference pixel value (pixels)', 
     &       status)
        
        call modifyn_f_kw(data_unit, 'TCRVL', y_det_col, 0.0, 
     &       decimals, 'Detector Y reference pixel value (pixels)', 
     &       status)
