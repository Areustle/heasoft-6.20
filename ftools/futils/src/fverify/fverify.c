		...
		)
{
    ...
    *status = 0;
    ...
    /* Print the header? */
    strcpy(parname,"prhead");
    Uclgsb(parname,&prhead,status);
    if(*status) {
        c_fcerr("could not get the prhead parameter");
        return *status;
    }
 
    /* Print out the prstat */
    strcpy(parname,"prstat");
    Uclgsb(parname,&prstat,status);
    if(*status) {
        c_fcerr("could not get the prstat parameter");
        return *status;
    }

    /* How many errors do you want to report */
    err_report = 0;
    strcpy(parname,"errreport");
    Uclgst(parname,temp,status);
    if(*status) {
        c_fcerr("could not get the errreport parameter");
        return *status;
    }
    p = temp;
    /* determine 'Severe error", "Error", or "Warning" report level */
    if( *p == 's' || *p == 'S') err_report = 2;
    if( *p == 'e' || *p == 'E') err_report = 1;

    /* test the data? */
    strcpy(parname,"testdata");
    Uclgsb(parname,&testdata,status);
    if(*status) {
        c_fcerr("could not get the testdata parameter");
        return *status;
    }

    /* test the checksum? */
    strcpy(parname,"tchksum");
    Uclgsb(parname,&testcsum,status);
    if(*status) {
        c_fcerr("could not get the tchksum parameter");
        return *status;
    }

    /* test the bytes in the fill area? */
    strcpy(parname,"testfill");
    Uclgsb(parname,&testfill,status);
    if(*status) {
        c_fcerr("could not get the testfill parameter");
        return *status;
    }


    /* Check the heasarc convention? */
    strcpy(parname,"heasarc");
    Uclgsb(parname,&heasarc_conv,status);
    if(*status) {
        c_fcerr("could not get the heasarc parameter");
        return *status;
    }
    ...
    if(*status) {
        c_fcerr("could not get the clobber parameter");
        return *status;
    }
    if(clobber) { 
      if(outfile[0] != '!') { 
    ...
    void update_parfile(int nerr, int nwrn)
{
    ...
    Uclpsi(parname, totalwrn, &status);
    if(status) {
       c_fcerr("Error to update the numwrns keyword.");
       status = 0;
    }
    strcpy(parname, "numerrs");
    Uclpsi(parname, totalerr, &status);
    if(status) {
       c_fcerr("Error to update the numerrs keyword.");
       status = 0;
    }
}
