...
#ifdef unix
#ifdef sun
void xclock1_(count,total,step)
long *count, *total, *step;
#else
void xclock1_(int *count,int *total,int *step)
#endif
#define OUTDEV "/dev/tty"
#endif

#ifdef vms
void xclock1(long int *count,long int *total,long int *step)
#endif
{
  ...
  if (*step <= 100 && *step > 0 && *count > 0 && *count <= *total)
      ...
      if (value % *step == 0 && value != pvalue && *count != *total)
	{
	  ...
	  fprintf(stdout,"%3ld%% completed\r",pvalue = value);
	  fflush(stdout);
	}
      else if (*count == *total)
        {	
