      subroutine xrftgtky(lui,ichat,iopt,nrows,itype,ivect,htunit,ctunit
...
c    This warning is also supressed by ichat = 0
...
c     TIMERPIX keyword.  Its value should be between 0 and 1, and
...
c     0.5 from dfoffset before returning it.
...
c Author: eal  HSTX/GSFC HEASARC  March, 1994
     ...
     &   ,iopt(*),nrows,npts,kystat,ivect(*),frow,ichat
      DOUBLE PRECISION mjdref,dtzero,dtint,dtoffset,dtsta,dtsto,dfoffset
     &   ,ddum,nulvd
      data nulvd /-1.2d34/
...
c ---------------------------------------------------------------------
      ...
      kystat = 0
      CALL ftgkyd(lui,'TIMERPIX',dfoffset,comm,kystat)
      IF(kystat.eq.0) then
         dfoffset = dfoffset - 0.5d0
      ELSE
         dfoffset = 0.d0
      ENDIF
