...
C
C DESCRIPTION:
C      Gets the number of real*4 ranges and their limits
...
C      EAG 9/7/94 Return with abs_min:abs_max if rows if ' ' or -
C
C NOTES:
C
C USAGE:
C      call fcgrgf(rows,abs_min,abs_max,numranges,rowrange1,rowrange2)
C
C ARGUMENTS:
...
C
C******************************************************************************
      subroutine fcgrgf(rows,abs_min,abs_max,numranges,range1,range2)

      character*(*) rows
      integer numranges
      real*4 range1(15), range2(15), abs_min, abs_max
