*+RDKEYCOLE
c     ---------------------------------------------------------------
      subroutine rdkeycole(iunit,ttype,ncol,nrows,name,defval,
...
c errflg  (o) integer         : Error status, 0 okay, 1 column and keyword
...
c
      rnull = 0
      errstr = ' ERROR: RDKEYCOLE Ver '//version//':'
      len = fcstln(errstr)
      qcol = .false.
      errflg = 0
      do i=1,ncol
...
c
      IF (qcol) THEN
        ...
        ELSE
          status = 0
          call ftgcve(iunit,colno,1,1,nrows,rnull,colvals,
     &                anyflg,status)
      ...
      ELSE
        status = 0
        call ftgkye(iunit,name,colvals(1),comm,status)
