...
if test x"${no_tcl}" = x ; then
  ...
  if test x"${ac_cv_c_tclconfig}" = x ; then
    for i in \
		../tcl \
		`ls -dr ../itcl2.2/tcl[[7-9]]* 2>/dev/null` \
		../../tcl \
...
if test x"${no_tcl}" = x ; then
  # we reset no_tcl in case something fails here
  no_tcl=true
  AC_ARG_WITH(itclconfig, [  --with-itclconfig           directory containing tcl configuration (itclConfig.sh)],
  ...
  if test x"${ac_cv_c_itclconfig}" = x ; then
    ITCLCONFIG="# no Tcl configs found"
    AC_MSG_WARN(Can't find Tcl configuration definitions)
  else
    no_tcl=
...
dnl    AC_SUBST(ITCL_EXEC_PREFIX)
    ...
    AC_SUBST(ITCL_SHLIB_LD)
dnl don't export, not used outside of configure
dnl AC_SUBST(ITCL_SHLIB_LD_LIBS)
dnl AC_SUBST(ITCL_SHLIB_SUFFIX)
dnl not used, don't export to save symbols
dnl AC_SUBST(ITCL_DL_LIBS)
    AC_SUBST(ITCL_LD_FLAGS)
dnl don't export, not used outside of configure
dnl AC_SUBST(ITCL_LD_SEARCH_FLAGS)
dnl AC_SUBST(ITCL_COMPAT_OBJS)
    AC_SUBST(ITCL_RANLIB)
    AC_SUBST(ITCL_BUILD_LIB_SPEC)
    AC_SUBST(ITCL_LIB_SPEC)
dnl AC_SUBST(ITCL_LIB_VERSIONS_OK)

dnl not used, don't export to save symbols
dnl    AC_SUBST(ITCL_SHARED_LIB_SUFFIX)

dnl not used, don't export to save symbols
dnl    AC_SUBST(ITCL_UNSHARED_LIB_SUFFIX)
])
...
if test x"${no_tk}" = x ; then
  ...
  if test x"${ac_cv_c_tkconfig}" = x ; then
    for i in \
		../tk \
		`ls -dr ../itcl2.2/tk[[4-9]]* 2>/dev/null` \
		../../tk \
...
if test x"${no_itk}" = x ; then
  ...
  if test x"${ac_cv_c_itkconfig}" = x ; then
    for i in \
		../itk \
		`ls -dr ../itcl2.2/itk 2>/dev/null` \
		../../itk \
		`ls -dr ../../itk[[4-9]]* 2>/dev/null` \
		../../../itk \
		`ls -dr ../../../itk[[4-9]]* 2>/dev/null` ; do
      if test -f "$i/unix/itkConfig.sh" ; then
        ac_cv_c_itkconfig=`(cd $i/unix; pwd)`
	break
      fi
    done
  fi
  ...
  if test x"${ac_cv_c_itkconfig}" = x ; then
    for i in \
		${srcdir}/../itk \
		`ls -dr ${srcdir}/../itk[[4-9]]* 2>/dev/null` ; do
      if test -f "$i/unix/itkConfig.sh" ; then
        ac_cv_c_itkconfig=`(cd $i/unix; pwd)`
	break
      fi
