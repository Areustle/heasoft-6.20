...
C      and xtetape utilities. Unix version.
C
C     Author:  Don Jennings, Code 631, NASA/HSTX
C     Date:    05/04/94
C     Version: 1.3
C
C     This module contains 10 functions:
C
...
C
      ...
      do i = 1,NUM_SUBDIRS
         ...
         dir = load_dir(1:space) // subdir(1:slash)
...
C 
      subroutine read_file(filename,position,verbose,status)
C
      implicit      none
      character*(*) filename
      character(80)  context
     ...
     *              createfile,readfile,closefile,space
      logical*4     verbose
      common        /tapeio/ dp,fp,pos
C
