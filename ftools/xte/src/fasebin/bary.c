char *bcrcsid = "RCS: $Id: bary.c,v 1.8 2005/06/03 19:11:49 miket Exp $" ;
/*-----------------------------------------------------------------------
 *
 *  bary.c
 *
...
FILE *openAFile (const char *name)
{
  ...
  if ( filepath == NULL ){
    sprintf (barymsg, "Neither $%s, nor $%s is defined", TIMINGDIR, LHEADIR) ;
    c_fcerr(barymsg);
  }
  else if ( FF == NULL ){
    sprintf (barymsg, "File %s could not be found in %s", fileName, filepath) ;
    c_fcerr(barymsg);
  }
 ...
 */
  if (numleaps == 0) {
	...
	printf("Baryinit: cannot handle more than %i leapseconds. Please adapt MAX_LEAP in bary.h and recompile.",MAX_LEAPS);
	exit(-5);
      }
    }
    fclose (FF) ;
 ...
 */
  if ( ephnum >= 0 ) {
    if ( i = initephem (ephnum, &denum, &c, &radsol, &msol) ) {
      sprintf (barymsg, "Error while initializing ephemeris; status: %d",
	       i) ;
      c_fcerr(barymsg);
      denum = 0 ;
    }
 ...
 */
  if ( scposn == NULL ) {
    sprintf (barymsg, "barycorr: Invalid Observatory/Spacecraft position vector") ;
    c_fcerr(barymsg);
    return 0.0 ;
  }
 ...
 */
  ...
  if ( (eposn = dpleph (jdt, iearth, isun)) == NULL ) {
    sprintf (barymsg, "barycorr: Could not find solar system ephemeris for MJD %f",
	     mjdTT->MJDint) ;
    c_fcerr(barymsg);
    return 0.0 ;
  }
 ...
 */
  ...
  if ( ( PD = openAFile (PSRTIME) ) == NULL ) {
    sprintf (barymsg, "Could not open file %s", PSRTIME) ;
    c_fcerr(barymsg);
    error = 1 ;
  }
 ...
 */
  ...
  if ( error > 1 ){
      sprintf (barymsg, "Could not find %s in file %s", insource, PSRTIME) ;
      c_fcerr(barymsg);
  }
 ...
 */
    ...
    ptp->denum = tdenum ;
    if ( debug ){
	sprintf (barymsg,"Source: %s, RA = %.8f,  Dec = %.8f,  Frame = DE%d",
    ...
    if ( denum != ptp->denum ) {
      sprintf (barymsg, "Warning: You are using Solar System Ephemeris DE%d",
	       denum) ;
      c_fcerr(barymsg);
      sprintf (barymsg, "         with a Timing Ephemeris produced with DE%d",
	       ptp->denum) ;
      c_fcerr(barymsg);
      if ( ( denum == 405 ) && ( ptp->denum == 200 ) ) {
	  c200to405 (dir, debug) ;
	  sprintf (barymsg, "         The coordinates will be transformed to DE405") ;
	  c_fcerr(barymsg);
      }
    }
    ptp->f = fd ;
 ...
 */
    ...
    toTT (&mtime) ;
    if ( debug ) {
      sprintf (barymsg,"Data MJD(TDB): %d", mjdt) ;
      c_fcecho(barymsg);
      sprintf (barymsg,"Used radio pulsar database entry covering the period:") ;
      c_fcecho(barymsg);
      sprintf (barymsg,"  MJD(TDB) %d to %d; rms: %f milliperiod", mjd1, mjd2, rms) ;
      c_fcecho(barymsg);
      if ( nocover ){
	  sprintf (barymsg,"===> Note that no entry could be found covering the observations.") ;
	  c_fcecho(barymsg);
      }
      sprintf (barymsg,"Frequency/period ephemeris (T0):") ;
      c_fcecho(barymsg);
      sprintf (barymsg,"  MJD(TDB) = %d + %g", mjdi, mjdftrun) ;
      c_fcecho(barymsg);
      sprintf (barymsg,"  f = %.14f  df/dt = %g  d2fdt2 = %g  d3fdt3 = %g", fd, dfd, ddfd, dddfd) ;
      c_fcecho(barymsg);
      sprintf (barymsg,"Radio pulse TOA ephemeris (Tr):\n  MJD(TT)  = %d + %g",
 ...
 */
  ...
  if ( bin && !error ) {
    if ( ( PD = openAFile (PSRBIN) ) == NULL ) {
      sprintf (barymsg, "Could not open file %s", PSRBIN) ;
      c_fcerr(barymsg);
      error = -1 ;
    }
 ...
 */
    ...
    if ( error < -1 ){
      sprintf (barymsg, "Could not find %s in file %s", insource, PSRBIN) ;
      c_fcerr(barymsg);
    }
 ...
 */
	...
	if ( line[i] == 'D' )
      ...
      if ( debug ) {
	sprintf (barymsg," Binary orbit (%s):", binsource) ;
	c_fcecho(barymsg);
	sprintf (barymsg,"  MJD = %d + %g",
		pbp->t0.MJDint, pbp->t0.MJDfr) ;
	c_fcecho(barymsg);
	sprintf (barymsg,"  pb = %g  a1sini = %g  e = %g",
 ...
 */
  if ( error > 0 || nocover ) {
    sprintf (barymsg, "===>  Please note that a suitable timing ephemeris entry") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      could not be found in the pulsar timing database.") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      You may want to get a fresh copy of:") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "        ftp://pulsar.princeton.edu/gro/jcat") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      and deposit it in $TIMING_DIR/psrtime.dat, ") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      or you may want to make your own timing ephemeris and") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      put it in the proper format  into that file.") ;
    c_fcerr(barymsg);
  }
  else if ( error ) {
    sprintf (barymsg, "===>  Please note that a suitable binary orbit ephemeris entry") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      could not be found in the pulsar timing database.") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      You may want to get a fresh copy of:") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "        ftp://pulsar.princeton.edu/gro/psrbin.dat") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      and deposit it in $TIMING_DIR/psrbin.dat, or you may want") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      to make your own binary orbit ephemeris and put it in") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      the proper format into that file.") ;
    c_fcerr(barymsg);
  }
  if ( fbin ) {
    sprintf (barymsg, "===>  Please note that the pulsar database indicated that this pulsar") ;
    c_fcerr(barymsg);
    sprintf (barymsg, "      is a binary pulsar and faseBin will try to treat it as such.") ;
    c_fcerr(barymsg);
    error = - 3 ;
  }
  return error ;
...
void c200to405 (double *dir, int debug) {
  ...
  if ( debug ) {
    sprintf (barymsg,"Conversion      DE200     to    ICRS:") ;
    c_fcecho(barymsg);
    sprintf (barymsg,"   X         %12.9f    %12.9f", dir200[0], dir[0]) ;
    c_fcecho(barymsg);
    sprintf (barymsg,"   Y         %12.9f    %12.9f", dir200[1], dir[1]) ;
    c_fcecho(barymsg);
    sprintf (barymsg,"   Z         %12.9f    %12.9f", dir200[2], dir[2]) ;
    c_fcecho(barymsg);
  }
  return ;
}
