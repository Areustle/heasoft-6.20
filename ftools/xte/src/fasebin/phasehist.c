char *phrcsid = "RCS: $Id: phasehist.c,v 1.9 2000/12/14 19:18:16 miket Exp $" ;
/*-----------------------------------------------------------------------
 *
 *  phaseHist.c
 *
 ...
 */
char *phProgName = {"phaseHist $Revision: 1.9 $"} ;
fitsfile *data_file;
int htype ;
char timesys[80] = "UTC" ;
char filetype[80] ;
...
int readhead (char *, char *, char *, int *, char *, int) ;
 ...
 */
  if ( fits_open_file (&data_file, dataFile, 0, &error) ) {
    sprintf(phmsg, "%s: cannot open file %s", phProgName, dataFile) ;
    c_fcerr(phmsg);
    return hist ;
  }
  fits_movabs_hdu (data_file, 2, &htype, &error) ;
  if (debug){
      sprintf (phmsg,"Data file: %s, htype: %d, error: %d", dataFile, htype, error) ;
      c_fcecho(phmsg);
  }
 ...
 */
  error = readhead (telescope, instrument, datobs, gainapp, dataFile, debug) ;
  if ( error ) {
    sprintf (phmsg, "%s: Error reading header of %s; status %d",
 ...
 */
  ...
  if ( debug ){
    sprintf (phmsg, "%s: initialized - %d phase and %d channel bins",
 ...
 */
  if ( !error ) {
    if ( !strcmp(timesys, "TT") )
      ...
      sprintf (phmsg, "%s: unrecognized TIMESYS: %s; will assume it is UTC",
	       phProgName, timesys) ;
      c_fcerr(phmsg);
      bcorr = 1 ;
    }
    if ( debug ){
	sprintf (phmsg, "%s: timesystem: %s", phProgName, timesys) ;
	c_fcerr(phmsg);
    }
    if ( !bcorr ) {
    ...
    denum = baryinit (mission, timesys, toTDB, mjdrefi, mjdreff, timeunit,
		      ephnum) ;
    if ( denum ) {
      sprintf (phmsg,"%s: Using JPL Planetary Ephemeris DE-%d", phProgName, denum) ;
      c_fcecho(phmsg);
      if ( debug ){
	  sprintf (phmsg, "%s: bary stuff initialized", phProgName) ;
	  c_fcerr(phmsg);
      }
    }
    else {
      sprintf (phmsg, "%s: Could not initialize bary stuff", phProgName) ;
      c_fcerr(phmsg);
      error = -20 ;
    }
 ...
 */
  ...
  if ( !error ) {
	...
	binary = 0 ;
	  ...
	  sprintf (phmsg, "%s: Could not find source %s for MET %d",
	...
	sprintf (phmsg, "%s: Could not find source %s in binary pulsar file",
		 phProgName, source) ;
	c_fcecho(phmsg);
	sprintf (phmsg, "%s: Proceeding without binary orbit corrections",
		 phProgName) ;
	c_fcecho(phmsg);
	error = 0 ;
	binary = 0 ;
 ...
 */
  ...
  if ( !error ) {
    ...
    if ( debug ) {
	sprintf (phmsg, "%s: TSTART = %f, TSTOP = %f",
		 phProgName, t1, t2) ;
	c_fcerr(phmsg);
	for (i=0; i<nTR; i++) {
	    sprintf (phmsg, "%s: %d start %f stop %f",
		     phProgName, i, starts[i], stops[i]) ;
	    c_fcerr(phmsg);
	    if ( i < ngti ){
		sprintf (phmsg, "  newstart %f newstop %f",
			 start[i], stop[i]) ;
		c_fcerr(phmsg);
	    }
	    else
    ...
    if ( !ngti ){
	sprintf (phmsg, "%s: no valid time ranges from %g to %g",
	       phProgName, t1, t2) ;
	c_fcecho(phmsg);
    }
    else {
	...
	*tstop += xbarycorr(*tstop, dir, xscorbit(orbitFile, *tstop, &error))
      ...
      if ( debug ) {
	sprintf (phmsg, "%s: ngti = %d, tstart = %f, tstop = %f, exposure = %f",
		 phProgName, ngti, start[0], stop[ngti-1], totexp) ;
	c_fcerr(phmsg);
	sprintf (phmsg, "%s: in TDB: tstart = %f, tstop = %f",
...
/*        Time column first; add tZero; if outside range, forget it  */
    ...
    if ( Time[nbufrow-1] < start[igti] ) {
      row += nbufrow ;
      if ( debug ) {
	sprintf (phmsg, "%s: read %d rows, starting at %d, skipping",
		 phProgName, nbufrow, row+1) ;
	c_fcerr(phmsg);
	sprintf (phmsg, "               start = %f, stop = %f",
    ...
    if ( Time[0] > stop[ngti-1] ) {
      row += nbufrow ;
      if ( debug ) {
	sprintf (phmsg, "%s: read %d rows, starting at %d, stopping",
		 phProgName, nbufrow, row+1) ;
	c_fcerr(phmsg);
	sprintf (phmsg, "               start = %f, stop = %f",
		 Time[0], Time[nbufrow-1]) ;
...
/*        The other columns - by mission  */
    ...
    if ( debug ){
	sprintf (phmsg, "%s: read %d rows, starting at %d after %d events",
 ...
 */
	phase = xabsphase (t,  &ptp, binary, &pbp, bcorr, dir, orbitFile,
			  &error) + ddp ;
	phase -= (int) phase ;
	/* MJT debug is boolean in ftools version and set to 1 for yes
	 * in fasebin.c (line 470). Change it there to trigger these phmsgs */
	if ( debug > 7 ) { 
	  sprintf (phmsg, "TB = %18.17g   ", t) ;
	  c_fcerr(phmsg);
	  sprintf (phmsg, "Phase = %6.3g", phase) ;
	  c_fcerr(phmsg);
	}
	j = phase * dnphase ;
	hist[chan][j]++ ;
 ...
 */
	...
	if ( simevent ) {
	  for (i=0; i<pfnphbin; i++) {
	    ...
	    phase -= (int) phase ;
	    if ( debug && !i ) {
	      sprintf (phmsg, "TB = %18.17g   ", tt) ;
	      c_fcerr(phmsg);
	      sprintf (phmsg, "First bin phase = %6.3g", phase) ;
	      c_fcerr(phmsg);
	    }
	    j = phase * dnphase ;
 ...
 */
	  ...
	  for (i=0; i<pfnphbin; i++) {
	    ...
	    if ( debug && !i ) {
	      sprintf (phmsg, "TB = %18.17g   ", tt) ;
	      c_fcerr(phmsg);
	      sprintf (phmsg, "First bin phases = %6.3g to %6.3g", yy1, yy2) ;
	      c_fcerr(phmsg);
	    }
	    jj1 = (int) yy1 ;
	    jj2 = (int) yy2 + 1 ;
 ...
 */
  if ( debug ){
      sprintf (phmsg, "%s: before finishing up, error status %d",
	       phProgName, error) ;
      c_fcerr(phmsg);
  }
  switch (dtype) {
 ...
 */
  ...
  if ( debug ){
      sprintf (phmsg, "%s: finished; accepted %d events",
	       phProgName, nevents) ;
      c_fcerr(phmsg);
  }
 ...
 */
  ...
  if ( error && !accum ) {
    if ( debug ){
	sprintf (phmsg, "%s: error %d occurred, freeing hist",
		 phProgName, error) ;
	c_fcerr(phmsg);
    }
    while ( hist[i] )
...
int readhead (char *telescope, char *instrument, char *datobs,
	      int *gainapp, char *dataFile, int debug)
/*------------------------------------------------------------------
 *
 ...
 */
  ...
  if (debug){
      sprintf (phmsg,"EXTNAME: %s, status: %d", filetype, stat) ;
      c_fcecho(phmsg);
  }
  if ( stat ) {
  ...
  if (debug){
      sprintf (phmsg,"TSTART: %f, status: %d", t1, stat) ;
      c_fcecho(phmsg);
  }
  fits_read_key   (data_file, TDOUBLE, "TSTOP",    &t2,        line, &stat) ;
  if (debug){
      sprintf (phmsg,"TSTOP:  %f, status: %d", t2, stat) ;
      c_fcecho(phmsg);
  }
  fits_read_key   (data_file, TINT,    "NAXIS2",   &nrows,     line, &stat) ;
  if (debug){
      sprintf (phmsg,"NAXIS2: %d, status: %d", nrows, stat) ;
      c_fcecho(phmsg);
  }
  sprintf (phmsg,"Working on data file %s", dataFile) ;
  c_fcecho(phmsg);
  sprintf (phmsg,"  Configuration %s", config) ;
  c_fcecho(phmsg);
 ...
 */
  if ( !strcmp(filetype, "XTE_SE") ) {
    ...
    sprintf (phmsg, "%s: unrecognized EXTNAME: %s", phProgName, filetype) ;
    c_fcerr(phmsg);
    dtype = INVALID ;
    error = -1 ;
  }
  ...
  if ( !error ) {
	...
	if ( !strncmp(config, "PF_", 3) )
	  ...
	  sprintf (phmsg, "%s: invalid configuration/EXTNAME pair:\n%s  %s",
	...
	if ( strstr(config, "E_") )
      ...
      error = -4 ;
      sprintf (phmsg, "%s: invalid instrument: %s",
	       phProgName, ddroot) ;
      c_fcerr(phmsg);
    }
  }
  ...
  if ( debug )
    switch ( dtype ) {
    case PCA_PF:
      sprintf (phmsg, "%s: dtype PCA_PF", phProgName) ;
      c_fcerr(phmsg);
      break ;
    case PCA_B:
      sprintf (phmsg, "%s: dtype PCA_B", phProgName) ;
      c_fcerr(phmsg);
      break ;
    case PCA_SB:
      sprintf (phmsg, "%s: dtype PCA_SB", phProgName) ;
      c_fcerr(phmsg);
      break ;
    case PCA_GX:
      sprintf (phmsg, "%s: dtype PCA_GX", phProgName) ;
      c_fcerr(phmsg);
      break ;
    case PCA_SE:
      sprintf (phmsg, "%s: dtype PCA_SE", phProgName) ;
      c_fcerr(phmsg);
      break ;
    case HEXTE_SE:
      sprintf (phmsg, "%s: dtype HEXTE_SE", phProgName) ;
      c_fcerr(phmsg);
      break ;
    default:
      sprintf (phmsg, "%s: illegal dtype %d", phProgName, dtype) ;
      c_fcerr(phmsg);
      break ;
    }
  return ;
 ...
 */
  ...
  if ( debug )
    switch ( dtype ) {
    case ACIS_SE:
      sprintf (phmsg, "%s: dtype ACIS_SE", phProgName) ;
      c_fcerr(phmsg);
      break ;
    case HRC_SE:
      sprintf (phmsg, "%s: dtype HRC_SE", phProgName) ;
      c_fcerr(phmsg);
      break ;
    case AXAF_SE:
      sprintf (phmsg, "%s: dtype AXAF_SE", phProgName) ;
      c_fcerr(phmsg);
      break ;
    default:
      sprintf (phmsg, "%s: illegal dtype %d", phProgName, dtype) ;
...
/*  Adjust for frequency errors  */
	...
	if ( debug ){
	    sprintf (phmsg, "subline: %s, t = %f, deltapf = %f, i = %d",
		     subline, t, deltapf, i) ;
	    c_fcerr(phmsg);
	}
	pfper += i * deltapf ;
	subline = strstr (subline, ",") ;
      ...
      if ( debug ){
	  sprintf (phmsg, "pfper = %f, ipfper = %d, timedel = %f; ",
		   pfper, ipfper, timedel) ;
	  c_fcerr(phmsg);
      }
      pfper *= ipfper ;
      if ( debug ){
	  sprintf (phmsg, "pfper = %f", pfper) ;
	  c_fcerr(phmsg);
      }
    }
    break ;
  case PCA_GX:
    if ( l1only > 0 ) {
      sprintf (phmsg,"===>Note: Only layer 1 good xenon events will be selected") ;
      c_fcecho(phmsg);
      sprintf (phmsg,"          from configuration %s", config) ;
      c_fcecho(phmsg);
    }
    else if ( l1only == 0 ) {
      sprintf (phmsg,"===>Note: All good xenon events will be selected") ;
      c_fcecho(phmsg);
      sprintf (phmsg,"          from configuration %s", config) ;
      c_fcecho(phmsg);
    }
	...
	sprintf (phmsg,"===>Note: Only PROPANE events will be selected") ;
	c_fcecho(phmsg);
	sprintf (phmsg,"          from configuration %s", config) ;
	c_fcecho(phmsg);
      }
      else {
	sprintf (phmsg,">>>>Note: You asked for PROPANE events to be selected") ;
	c_fcecho(phmsg);
	sprintf (phmsg,">>>>      from configuration %s", config) ;
	c_fcecho(phmsg);
	sprintf (phmsg,">>>>      Hence, this data file will be skipped") ;
	c_fcecho(phmsg);
	error = -10 ;
      }
    }
  ...
  case PCA_SE:
	...
	if ( debug ){
	    sprintf (phmsg, "%s:%d characters left in str", phProgName, strlen(str)) ;
	    c_fcerr(phmsg);
	}
	for (i=0; str[i]; i++)
	    str[i] = str[i+1] ;
	if ( debug ){
	    sprintf (phmsg, "%s: stopped at character %d in str", phProgName, i) ;
	    c_fcerr(phmsg);
      }
    }
    n = k = 0 ;
    if ( debug ){
	sprintf (phmsg, "%s: TEVTB2: %s",
		 phProgName, evtb2) ;
	c_fcerr(phmsg);
    }
    str = evtb2 ;
	...
	chanmask = 2 * chanmask + 1 ;
      if ( debug ){
	  sprintf (phmsg, "%s: PCA_SE: found %d bits before T, %d pha bits, chmask %d",
  ...
  if ( strcmp (cpix, "none") ) {
	...
	error = -5 ;
	sprintf (phmsg, "%s: new CPIX (%s) differs from previous CPIX (%s)",
    ...
    strcpy (cpix, newcpix) ;
  if ( debug ){
      sprintf (phmsg, "%s: CPIX = %s",
	     phProgName, cpix) ;
      c_fcerr(phmsg);
  }
  if ( debug ){
      sprintf (phmsg, "%s: numchan = %d",
	       phProgName, numchan) ;
      c_fcerr(phmsg);
  }
  if ( debug && pfnbin ){
      sprintf (phmsg, "%s: pfnbin = %d, pfnphbin = %d, pfnchbin = %d",
 ...
 */
  ...
  if ( !error ) {
    ...
    if ( error ) {
      sprintf (phmsg, "%s: cannot find PHA column", phProgName) ;
      c_fcerr(phmsg);
      error = 0 ;
      fits_get_colnum (data_file, CASEINSEN,   "pi", &datacol, &error) ;
      if ( error ) {
	sprintf (phmsg, "%s: cannot find PI column", phProgName) ;
	c_fcerr(phmsg);
	*nchan = 1 ;
	error = 0 ;
      }
    ...
    if ( error ){
	sprintf (phmsg, "%s: cannot find Time column", phProgName) ;
	c_fcerr(phmsg);
    }
  }
  if ( error ){
      sprintf (phmsg, "%s: cannot operate on this file", phProgName) ;
      c_fcerr(phmsg);
  }
  else {
    if ( datacol ) {
