/* $Id: scorbit.c,v 1.3 2000/12/14 19:18:16 miket Exp $ */
/*------------------------------------------------------------------
 *
 *  File:        scorbit.c
 ...
 */
  *oerror = 0 ;
  ...
  if ( strcmp (savefile, filename) ) {
	...
	sprintf(scomsg, "xscorbit: Cannot open file %s", filename) ;
	c_fcerr(scomsg);
    }
    else {
 ...
 */
  ...
  if ( ( startinx <= 0 ) || ( startinx > numrows-2 ) ) {
    sprintf(scomsg, "xscorbit: Time %g not covered by file %s", t, filename) ;
    c_fcerr(scomsg);
    *oerror = -2 ;
    return intposn ;
  }
 ...
 */
  ...
  if ( !nbrows || !startrow || ( relinx > nbrows-3 ) || (relinx < 0 ) ) {
    ...
    if ( *oerror ) {
      sprintf (scomsg, "xscorbit: error while reading rows starting at %d",
	       startrow) ;
      c_fcerr(scomsg);
      return intposn ;
    }
