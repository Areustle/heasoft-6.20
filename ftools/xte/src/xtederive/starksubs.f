...
C
C
      SUBROUTINE XTEFINDB0(STPS,BDEL,VALUE,BEQU,RR0)
C--------------------------------------------------------------------
...
C
C
      SUBROUTINE XTESHELLG(GLAT,GLON,ALT,FL,ICODE,B0)
C--------------------------------------------------------------------
...
c shut up compiler warnings (wonderfully written code forces
c double initializations thanks to entry points and gotos galore)
      iequ=0
      bequ=1.E10
      radik=0.0
...
c     MJT 26June2001 moved bequ initialization (see above)
      BEQU=1.E10
c these initializations have to be done both before and after
c the entry point to shut up the compiler warnings!
      radik=0.0
...
C
C
      SUBROUTINE XTEFELDG(GLAT,GLON,ALT,BNORTH,BEAST,BDOWN,BABS)        
C-------------------------------------------------------------------
...
C*****ENTRY POINT  FELDG  TO BE USED WITH GEODETIC CO-ORDINATES         
      ...
      SP=SIN(RLON)                                                
       ...
       YYY=RHO*SP                                               
      GOTO10                                                   
      ENTRY XTEFELDC(V,BNORTH,BEAST,BDOWN,BABS)                  
...
C
C
        SUBROUTINE XTEFELDCOF(YEAR,DIMO)
C------------------------------------------------------------------------
