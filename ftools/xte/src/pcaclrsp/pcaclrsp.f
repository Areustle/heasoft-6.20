...
C
C DESCRIPTION:      
C     Apply the PCA collimator correction to a light curve
C      
C AUTHOR:
...
C 
C 7/6/98 - by ZG to change the string length od obs_date and obs_time to 68
C	for y2k string format.
C Aug 18, 1999 - Replace gtcal with gtcalf (PDW)
C Aug 25, 1999 - changes in onequat, manyquat1,2 to fix bug in which null
C     input rate values were not trapped, and change to rd_quatrns2 to
C     extend accumulation of quaternions to TSTOP_IN + DEL_QUAT (another
C     bug fix).
C
C NOTES:
C
...
C      lcfil	 - name of input PCA light curve file
C      outfil    - name of output light curve file
C      xtefilt   - name of XTE filter file
C      collcube  - name fo PCA collimator cube file
C      pcu       - desired pcu (detectors 0-4, or 'all' for sum)
C      geomarea  - geometric area of desired pcu
...
c start with initial declarations
      ...
      taskname = 'pcaclrsp_v3.7'
      abort = .false.
        
c get the parameters from the par file
        call gpcaclrsp(lcfil, outfil, xtefilt, collcube,
     $     pcu, geomarea, jitter, ra, dec, start, stop, chatter)


c Perform the Algorithm:   
        call dopcaclrsp(lcfil, outfil, xtefilt, collcube,
     $       pcu, geomarea, jitter, ra, dec, start, stop, chatter)
...
C
C MODIFICATION HISTORY:
C     Oct 3, 1995 - Enhanced PCU specification so that either a single
C                   PCU may be specified or "all" may be specified
C     
C NOTES:
C      gpcaclrsp uses F77/VOS like calls to read parameters from .par file
C
C USEAGE:      
C      call gpcaclrsp(phafil, rmffil, arffil, xtefilt, collcube, pcu_str,
C                     geomarea, jitter, ra, dec, tstart, tstop, chatter)
C      
C ARGUMENTS:
C     lcfil	 - name of input PCA light curve file
C     outfil	 - name of output light curve file
...
C     pcu_str    - desired pcu (0 for sum)
...
C *******************************************************************************
      ...
      SUBROUTINE gpcaclrsp(lcfile, outfile, xtefilt, collcube,
     $     pcu_str, geomarea, jitter, ra, dec, tstart, tstop, chatter)


c start with the declarations
      character*(*) lcfile, outfile, xtefilt
      character*(*)  ra, dec, pcu_str
      character(160) collcube
      real geomarea(0:4), jitter
      integer chatter
...
C get the desired pcu (ALL should be acceptable input)
        call uclgst('pcu',pcu_str,status)
        if (status .ne. 0) then
           context = 'could not get PCU parameter'
           call fcerr(context)
	endif

C decode the pcu string get the pcu number.
c     Re - PCU's are numbered 0-4.  Internally, use 5 for 'all' detectors
c        call pars_pcu(pcu_str,pcu)
c        if (pcu .lt. 0 .or .pcu .gt. 5) then
        
C get the value of the geometric area of the desired pcu (hidden)
c        call uclgsr(area_pcu(pcu),geomarea(pcu),status)
c        if (status .ne. 0) then
c           context = 'could not get GEOM AREA parameter'
...
C
C USEAGE:      
C      call dopcaclrsp(phafil, rmffil, arffil, xtefilt, path,
C              pcu, geomarea, jitter, ra_str, dec_str, tstart, tstop, chatter)
C      
C ARGUMENTS:
C     lcfil	 - name of input PCA light curve file
C     outfil	 - name of output light curve file
...
C *******************************************************************************
      ...
      SUBROUTINE dopcaclrsp(lcfil, outfil, xtefilt, collcube,
     $     pcu_str, geomarea, jitter, ra_str, dec_str, tstart,
     $     tstop, chat)
...
C      character(16) obs_date, obs_time
	character(68) obs_date, obs_time
      ...
      character(8) taskname, runit, timeunit
...
c %%% Start of IMG Addition -------------
c Initalization
	wrnstr = '** DOPCACLRSP WARNING:'
	errstr2 = '** DOPCACLRSP ERROR:'
c %%% End of IMG Addition -------------
...
C From the lc file, obtain the source position [and the active PCUs]
...
C ... From the PCU number, determine the appropriate collimator cube
         call pars_pcu(pcu_str,pcu)
         if (pcu .lt. 0 .or. pcu .gt. 5) then
            ierr = 1
            context = errstr2//' invalid pcu value'
            go to 999
         endif         
         if (pcu .ge. 0 .and. pcu .le. 4) then
            detnam = 'PCU'//pcu_str
         else if (pcu .eq. 5) then
            detnam = 'all'
         endif
...
C
C DESCRIPTION:      
C     Parse a string of input pcu values to a pcu #.  PCU's are numbered
c      from 0-4. The value 'ALL' is returned with a value of 5.
C      
C AUTHOR:
...
C
C ARGUMENTS:
C     pcu_str    - character string of pcu values
C     pcu        - numerical value for pcu
C
C PRIMARY LOCAL VARIABLES:
C
C CALLED ROUTINES:
...
C ************************************************************************** 
      ...
      Subroutine pars_pcu(pcu_str,pcu)

c start with initial declarations
      character*(*) pcu_str
      integer pcu, ftstatus
      character(80) context
      
      ftstatus = 0

      if (pcu_str .eq. 'ALL' .or. pcu_str .eq. 'all') then
         pcu = 5
      else
         call ftc2i(pcu_str,pcu,ftstatus)
         if (ftstatus .ne. 0) then
            context = 'error converting pcu string to pcu number'
            call fcerr(context)
            go to 999
         end if         
      end if
