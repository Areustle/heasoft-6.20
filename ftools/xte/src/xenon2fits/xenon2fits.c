/*
  FILENAME:    xenon2fits.c
  purpose:     Translate Xenon Mode data 1, & 2 to SE FITS file
...
status:      
  ...
  02Feb2010: Handle TIMEPIXR keyword (courtesy of Craig Markwardt) (v4.0c)

  02Nov2011: Handle OGIP-style output keywords (v5.0a)
  06Nov2011:   ... and output TLMIN/MAX keywords for OGIP columns v5.0b
  */
...
/* The following are used as sample parameters when writing
   ...
   a standard ASCII dump of the data in the Xenon Mode data
   files.*/
...
static char ttype[MXELEM][FITS_CLEN_HDEFKWDS] = {
  "TIME", "PCUID", "PulseHeight", "Propane", "XL1", "XR1", "XL2", "XR2", "XL3", "XR3"
  };
...
/* OGIP column names */
static char ttypeo[MXELEM][FITS_CLEN_HDEFKWDS] = {
  "TIME", "Event", "PCUID", "ANODEID", "PHA"
  };
 
static char tformo[MXELEM][FITS_CLEN_HDEFKWDS] = {
    "D",  "24X",   "B",     "B",       "B"
  };

static char tunito[MXELEM][FITS_CLEN_HDEFKWDS] =  {
    "s",  "",      "",      "",        "chan"
  };
...
void Check_and_Write_Info_Xenon();
void Write_Events_to_Fits_Xenon();
int Read_Param_Xenon();
void Add_Event_A_Xenon();
void Write_Event_Xenon();

/* Subroutine XENON2FITS starts */
void Xenon2fits()
  {
  ...
  char xenon1[FitsStrBufLen+1], xenon2[FitsStrBufLen+1], 
  ...
  int ex1, ex2, ilun1, ilun2, i, ilunt1, ilunt2,
    ...
    test, pstat, nfield, nfieldx, nfieldo;
  double time1, time2;
...
/* Initial all variables that will be receiving information */
  ...
  XTE_Fcecho(" ");
  XTE_Fcecho("Running XENON2FITS version 5.0b");
  XTE_Fcecho("=============================================");

/* Read the Parameter file to get the input and output filenames. */

  status=Read_Param_Xenon(infile1,infile2,outfile,wrt_parm); 
  if(status !=0){
    XTE_Fcecho(" ");
    XTE_Fcecho("Could not complete Read_Param_Xenon call");
...
/* Notice that I have used PCINT in Fcpars */
  ...
  XTE_Fcecho("Reading in Good Xenon data files for 1, & 2");
  XTE_Fcecho(file1);
  XTE_Fcecho(file2);
  XTE_Fcecho(" ");
  ...
  if(wrt_parm[0] == 'O')XTE_Fcecho("Creating OGIP form of output file.");
...
/* Lets do a check to make sure that we are dealing with files of the 
  ...
  FCGKYS(ilun1,"DATAMODE",xenon1,comment1,&pstat);
  if(pstat != 0){
    XTE_Fcecho(" ");
  ...
  FCGKYS(ilun2,"DATAMODE",xenon2,comment2,&pstat);
  if(pstat != 0){
    XTE_Fcecho(" ");
...
/* Write out information that describes what the columns are */
  ...
  if(wrt_parm[0] == 'O'){
    ...
    /* Write out information that describes what the columns are */
    FCPHBN(olun,nrows,nfieldo,ttypeo,tformo,tunito,extnam,zero,&pstat);
    if(pstat != 0){
      XTE_Fcecho(" ");
      XTE_Fcecho("Could not write information for output file");
      Fcerrm(pstat);
...
/* Now that we actually have all of the absolutely necessary information 
  ...
  Check_and_Write_Info_Xenon(ilun1, ilun2, olun, wrt_parm);
...
/* Now we KNOW that formats for each of the input files and that never 
  ...
  while (runit){
    ...
    if(runit)
      Write_Events_to_Fits_Xenon(olun, &irow, &timesave, rowTimes[0], events, numberEvents, &runit, wrt_parm); 
...
/* Free assigned logical unit numbers*/
  ...
  XTE_Fcecho("Finished creating SE file from Good Xenon Mode data.");

  }

int Read_Param_Xenon(rfname1,rfname2,ofname,wrt_parm) 
...
/* Reset BufLen_2 to 9 since we have allocated 10 spaces to receive
  ...
  if(wrt_parm[0] != 'X' && wrt_parm[0] != 'A' && wrt_parm[0] != 'O'){
    XTE_Fcecho("Assign either X, ASCII or OGIP to wrtparm");
...
/*******************************************************
  ...
  ******************************************************/
void Add_Event_A_Xenon(ounit, irow, time, pulseHeight, propane, l1, r1, l2, r2, l3, r3, pcu)
     double time; 
     int pulseHeight, propane, l1, r1, l2, r2, l3, r3;
     int ounit;
     int irow; 
     int pcu;
{
...
/*******************************************************
  Add an OGIP-style event to the output FITS table
  ******************************************************/
void Add_Event_O_Xenon(ounit, irow, time, eventbits, pulseHeight, pcu, anode_id)
     double time; 
     int eventbits[24];
     int pulseHeight;
     int ounit;
     ...
     int pcu, anode_id;
{
  ...
  int stat=0,staterr=0;
  int felem=1, nelem=1, col=0, nbit=24;

  int lpulseHeight[1];
  int lpcu[1], lanodeid[1];
  char leventbits[24];
  int  xeventbits[24];
  int i;

  /* Reverse order for output!!! */
  for (i=0; i<24; i++) leventbits[i] = eventbits[23-i];
  for (i=0; i<24; i++) xeventbits[i] = eventbits[23-i];
  ...
  /* EVENT (bit word) */
  col++;
  FCPCLX(ounit,col,irow,felem,nbit,xeventbits,&stat);
  if(stat != 0){
    XTE_Fcecho(" ");
    XTE_Fcecho("Could not put Event Word information into outputfile");
    Fcerrm(stat);
    FCCLOS(ounit,&staterr);
    if(staterr != 0){
      XTE_Fcecho(" ");
  ...
  /* ANODEID */
  col++;
  FCPCLJ(ounit,col,irow,felem,nelem,lanodeid,&stat);
  if(stat != 0){
    XTE_Fcecho(" ");
    XTE_Fcecho("Could not put ANODEID information into outputfile");
    Fcerrm(stat);
    FCCLOS(ounit,&staterr);
    if(staterr != 0){
      XTE_Fcecho(" ");
...
********************************************************************/
void Write_Event_Xenon(ounit, irow, timesave, rowTime, micro, t1, t2, wrt_parm)
...
{
  double tt;
  int l1=0,l2=0,l3=0,r1=0,r2=0,r3=0, pcu=0, pulseheight=0;
  int anode_id = 0;
  int propane=0;
  int i,value[24];
...
/* Here we are calculating the amount of time that is to be added to the
   ...
   first is extracted from the Xenon mode 1 data (t1) and the last
   is gotten from the Xenon mode 2 data file (t2). This is added to 
   tt and than that is added to rowTime to calculate the total time. */
  micro += 0x1000 * (t1 & 0x01);
  micro += (t2 & 0x0fff);
...
/* Due to the way that FITSIO works it is VERY HARD to pass it a bit stream
 ...
 (4)      (2)      (1)
  -------------------     ---------------------------------------------       
          |                                    |                             
  P  C  U  I  D {3}      E[X3R   X3L      X2R      X2L      X1R     X1L] {6}  
 ...
 */
  ...
  pulseheight = (t1 & 0x01fe) >> 1;	/* Pulse Height Channel*/
  ...
  /*
    THIS IS A MODIFICATION!!!! FIX IT and TEST IT...
    We are setting a propane bit. */
  if ((t1 & 0x0e00) == 0x0c00){
    value[17] = 0x1;              /* VRP - Propane */
    propane = 1;
    anode_id = 40;
  }
...
/* If we want separate ASCII columns we use Add_Event_A_Xenon, if we want a 
   bit-stream column titled Event then we use Add_Event_X. */

  if(wrt_parm[0] == 'X')Add_Event_X(ounit, irow, tt, value);
  if(wrt_parm[0] == 'A')Add_Event_A_Xenon(ounit, irow, tt, pulseheight, propane, l1, r1, l2, r2, l3, r3, pcu);
  if(wrt_parm[0] == 'O')Add_Event_O_Xenon(ounit, irow, tt, value, pulseheight, pcu, anode_id);
...
/******************************************************************
  ...
  *****************************************************************/
void Write_Events_to_Fits_Xenon(ounit, irow, timesave, partTime, eventLists, numberEvents, runit, wrt_parm)
     ...
     char *wrt_parm;
{
  ...
  for (i=1; i< numberEvents[0] - 1; i++){
    ...
    if (wrong_order == 0){
	 ...
	 irowtemp++;
	 *irow=irowtemp;
	Write_Event_Xenon(ounit, irowtemp, timesave, partTime + delta, mtic,eventLists[0][i],eventLists[1][i], wrt_parm);
...
void Check_and_Write_Info_Xenon(ilun1, ilun2, olun, wrt_parm)
     ...
     char *wrt_parm;
{
  ...
  int fifteen=15;
  char xenon1[FitsStrBufLen+1],datamode[FitsStrBufLen+1],test;
  int ilen,ii,ihold=0;
  
  il[0]=ilun1, il[1]=ilun2;

  FCPKYS(olun,"ORIGIN","XTE-GOF","Created from Good Xenon 1,& 2 PCA data.",&pstat);
  if(pstat != 0){
    XTE_Fcecho(" ");
  ...
  if(wrt_parm[0] == 'X'){
    FCPKYS(olun,"CREATOR","XENON2FITSX","Created with supplied C code wrapped in FTOOLS format",&pstat);
    if(pstat != 0){
      XTE_Fcecho(" ");
  ...
  if(wrt_parm[0] == 'A'){
    FCPKYS(olun,"CREATOR","XENON2FITS","Created with supplied C code wrapped in FTOOLS format",&pstat);
    if(pstat != 0){
      XTE_Fcecho(" ");
      XTE_Fcecho("Could not set CREATOR keyword in output file");
      Fcerrm(pstat);
  ...
  if(wrt_parm[0] == 'O'){
    FCPKYS(olun,"CREATOR","XENON2FITSO","Created with XENON2FITS in OGIP output mode",&pstat);
    if(pstat != 0){
      XTE_Fcecho(" ");
      XTE_Fcecho("Could not set CREATOR keyword in output file");
      Fcerrm(pstat);
  ...
  FCGKYS(il[0],"DATAMODE",xenon1,comm,&pstat);
  if(pstat != 0){
    XTE_Fcecho(" ");
    XTE_Fcecho("Could not get DATAMODE information for file 1");
    XTE_Fcecho("Setting DATAMODE to read GoodXenon");
    strcpy(xenon1,"GoodXenonN_UNKNOWN");
    pstat=0;
  }
  ...
  */
  ...
  /*  for(ii=ilen-1; ii >= ihold; ii--){ */
    ...
    XTE_Fcecho(&xenon1[ii]); */
  /* if(xenon1[ii] == '_' || ii == ihold){ */
      /*      XTE_Fcecho("We got a match");
      XTE_Fcecho(datamode);       */
      /* strcat(datamode,&xenon1[ii]); */
      /*      XTE_Fcecho(datamode); */
  /*      ii = ihold-1;
    }
  }
  */
  ...
  for(ii=0; ii < ilen; ii++) {
    if(xenon1[ii] == '_'){
      test=xenon1[ii-1];
      if(isdigit(xenon1[ii-1]) != 0) {
	strcpy(datamode,"GoodXenon");
	/* xenon1[ii-1]='\0';
	strcat(datamode,xenon1); */
      } else {
	strcpy(datamode,"GoodXenonwithPropane");
      }
      ihold=ii;
      ii=ilen-1;
    }
  ...
  */
  ...
  FCGKYD(il[0],"TIMEPIXR",&dtemp,comm,&pstat);
  if(pstat != 0){
    XTE_Fcecho(" ");
    XTE_Fcecho("Could not get TIMEPIXR keyword from input file");
    XTE_Fcecho("Setting to 0.0d0");
    pstat=0;
    dtemp=0.0;
  }
  
  FCPKYD(olun,"TIMEPIXR",dtemp,14,comm,&pstat);
  if(pstat != 0){
    XTE_Fcecho(" ");
    XTE_Fcecho("Could not set TIMEPIXR keyword in output file");
    Fcerrm(pstat);
  }
...
/* Due to the question of precision, MJDREF may have to be broken up into
  ...
  FCPKYS(olun,"TDDES2","D[0~4] & E[X1L^X1R^X2L^X2R^X3L^X3R] & C[0~255]"," ",&pstat);
  if(pstat != 0){
    XTE_Fcecho(" ");
  ...
  if(wrt_parm[0] == 'X' || wrt_parm[0] == 'O'){
    ...
    if (wrt_parm[0] == 'O') {
      short int zero = 0, four = 4, ten = 10, forty = 40, tff = 255;
      FCPKYJ(olun,"TLMIN3",zero,"Minimum value for field",&pstat);
      FCPKYJ(olun,"TLMAX3",four,"Minimum value for field",&pstat);
      FCPKYJ(olun,"TLMIN4",ten, "Minimum value for field",&pstat);
      FCPKYJ(olun,"TLMAX4",forty,"Minimum value for field",&pstat);
      FCPKYJ(olun,"TLMIN5",zero,"Minimum value for field",&pstat);
      FCPKYJ(olun,"TLMAX5",tff, "Minimum value for field",&pstat);
      FCPKYS(olun,"CPIX5","0:255","PHA binning expression",&pstat);
    }
    ...
    FCPCOM(olun,"E[VPR] is the Propane bit",&pstat);
    if(pstat != 0){
      XTE_Fcecho(" ");
      XTE_Fcecho("Could not write out propane information.");
    }

    FCPCOM(olun,"It is either TRUE or FALSE",&pstat);
    if(pstat != 0){
      XTE_Fcecho(" ");
      XTE_Fcecho("Could not write out propane information.");
    }
    ...
    strcpy(s1,"(M[1]{1},S[Zero]{5},E[VPR]{1},D[0:4]{3},E[0:63]{6}");
    strcpy(s2,",C[0:255]{8})");
    strcat(s1,s2);
...
#ifdef vms
#define F77CALL xenon2fit
#endif
#ifdef unix
#define F77CALL xenon2fit_
#endif
