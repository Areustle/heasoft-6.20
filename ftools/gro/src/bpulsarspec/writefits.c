...
void  writefits(char *infname, char *bpulsfn, char *filename, 
      long *snrows, double *sumCts, double *sumCtsErr, double *texpos,
      int *decnum)
{
   ...
   /* define the name, datatype, and physical units for the 3 columns */
    char *ttype[] = { "CHANNEL", "COUNTS",    "STAT_ERR"      };
    char *tform[] = {  "I",        "1D",           "1E"        };
    char *tunit[] = {   "",       "counts",       "counts"    };
     ...
     if (qsys == 0)
      ...
      /* Write the data */
         ...
         if (status !=0)
	   {
          ...
          fits_write_col(outfptr, TDOUBLE, 2, 1, 1, 16, sumCts,
                   &status);
          if (status !=0)
	   {
            printf("Error writting channel counts!\n");
            exit(1);
	   }
 
          fits_write_col(outfptr, TDOUBLE, 3, 1, 1, 16, sumCtsErr,
                   &status);
          if (status !=0)
	   {
            printf("Error writting channel counts errors!\n");
            exit(1);
	   }
