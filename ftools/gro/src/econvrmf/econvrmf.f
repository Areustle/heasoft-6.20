C*****************************************************************************
C TASK NAME: econvrmf
C
C FILE NAME: econvrmf.f
C
...
C     subroutine econvf    - top level subroutine, called by IRAF or host
C                            C wrapper in heconvrmf.c
C     subroutine read_resp - reads in EGRET file from SPECTRAL
C     subroutine set_bins  - set up the incident energy bins
C     subroutine write_rsp - writes out FITS file in XSPEC format
C     subroutine cmprsp    - compress a response matrix to XSPEC format
C
...
C*****************************************************************************
      ...
      integer*4 MAX_GROUPS, MAX_TOT_GROUPS
...
C Setup common block for taskname - necessary for fcerr.
      ...
      status = 0
      call uclgst ('rspfil', rspfil, status)
      IF (status.NE.0) THEN
...
C Read in the EGRET response matrix data.
      call read_resp (rspfil, rspmatrix, resp_energies, ch_bounds,
     &                NENER, NCHAN)
...
C Write XSPEC response matrix.
      call write_rsp (NCHAN, NENER, MAX_ELEMENTS, MAX_GROUPS,
...
C
C CALLED ROUTINES:
C     subroutine set_bins - constructs incident bins
C  
C MODIFICATION HISTORY:
C     03/97, cosmetic changes, PJTL and JMSS, HSTX
C******************************************************************************

      subroutine read_resp (filnam, input_resp, resp_energies,
     &                      ch_bounds, nchan, nener)
...
C     RSP_MIN         minimum value of response that will be included
      integer*4 MAX_ENERGIES, MAX_CHANNELS
...
C Set up the incident energy bins.
      ...
      call set_bins (einc, ec, de, first, last)
      eobs2(0) = emin
...
C*****************************************************************************
      ...
      subroutine set_bins (einc, ec, de, first, last)
