C*****************************************************************************
C TASK NAME: speconvpha
C
C FILE NAME: speconvpha.f
C
...
C     subroutine speconva - top level subroutine, called by IRAF or host
C                         C wrapper in heconvpha.c
C     subroutine ecreadat - reads in EGRET file from SKYUTIL and LIKE
C     subroutine ecwftspf - writes out FITS file in XSPEC format
C
...
C
C ARGUMENTS:
C     specfil - SKYUTIL and LIKE .spec filename
C     phafil  - XSPEC .pha filename
C
...
C*****************************************************************************
      ...
      subroutine speconvpha (specfil, phafil, objnam)

      implicit none
 
      character*(*)   specfil, phafil, objnam
      ...
      character(80) sdsstr, edsstr, shsstr, ehsstr
...
C Read the EGRET .spec file produced by SKYUTIL and LIKE.
      open (77, file=specfil, status='old')
      call ecreadat (77, nchan, pha, errors, ranom, decnom, charlines)
      close (77)
...
C Write the FITS file.
      status = 0
      call ecwftspf (phafil, specfil, hist, nhist, assfls, equinx,
...
C******************************************************************************
      ...
      subroutine ecreadat (lusel, nregns, nmeas1, sigma1, ra, dec,
     &                   charlines)
...
C
C ARGUMENTS:
C     phafil     -  FITS filename
C     specfil    -  input PHA filename
...
C*****************************************************************************
      ...
      SUBROUTINE ecwftspf (phafil, specfil, hist, nhist, assfls, equinx,
     &                   sdsstr, shsstr, edsstr, ehsstr, objnam, tlscpe,
     &                   instrm, filter, ranom, decnom, roll, texpos, 
     &                   ascale, bscale, cscale, qgroup, group, qqual, 
     &                   qualty, nchan, dtype, chan, ipha, pha, qerror,
...
C Write out the additional keywords about the creation of the FITS file.
     ...
     &        'spectrum file contains time intervals and event', status)
      ...
      CALL ftpkys (ounit, 'FILENAME', specfil(:lenact(specfil)),
