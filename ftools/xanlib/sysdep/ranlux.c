/* routines to mimic the Fortran calls to ranlux.f. Modified from gsl code */

#include <stdlib.h>
#include "cfortran.h"

/* global definition of the struct carrying current state */
...
static unsigned long int ranlux_get ();
static double ranlux_get_double ();
static void ranlux_set_lux (unsigned long int s, unsigned int luxury);

void ranlux(float *rvec, int lenv);
void rluxgo(int lux, int seed, int k1, int k2);
...
void ranlux(float *rvec, int lenv)
{
  ...
  for (i=0; i<lenv; i++)
  {
    *(rvec+i) = (float)ranlux_get_double();
  }
  return;
}
FCALLSCSUB2(ranlux, RANLUX, ranlux, FLOATV, INT)
...
void rluxgo(int lux, int seed, int k1, int k2)
{
  ...
  ranlux_set_lux ((long)seed, luxury);
  return;
}

FCALLSCSUB4(rluxgo, RLUXGO, rluxgo, INT, INT, INT, INT)
...
static unsigned long int increment_state ();
...
ranlux_get_double ()
{
  return ranlux_get () / 16777216.0;
}

static void
ranlux_set_lux (unsigned long int s, unsigned int luxury)
{
  int i;
  long int seed;
