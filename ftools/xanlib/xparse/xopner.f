      SUBROUTINE XOPNER(CFILE, QPEND, CSTR, QTIME, IUNIT, IERR)
      CHARACTER CFILE*(*), CSTR*(*)
      LOGICAL   QPEND, QTIME
      INTEGER   IUNIT, IERR
C---
C XPARSE subroutine to open the error log file for output.
C---
...
C---
C 1991-Nov-21 - [AMTP] adaptation of xopnlg
...
C---
      ...
      NBUF=2
      LTMP=LENACT(CFILE)
      CALL ERRLOG(3, RBUF, NBUF, CFILE, LTMP)
      IF(NBUF.LT.0) RETURN
      IUNIT=RBUF(1)
C---
...
C         WRITE(CTMP,'(A,1X,A)') '!'//CSTR(:MAX(1,LTMP)),
C     :      CTIME(:MAX(1,LENACT(CTIME)))
         CTMP = '!'//CSTR(:MAX(1,LTMP))//' '//
     &           CTIME(:MAX(1,LENACT(CTIME)))
         LTMP=LENACT(CTMP)
         CALL ERRLOG(5, RBUF, NBUF, CTMP, LTMP)
      END IF
      RETURN
      END
