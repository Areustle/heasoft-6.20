...
/* $Id: udmget.c,v 1.18 2014/04/21 21:05:36 irby Exp $
   
   Explanatory comments added July 1999 Ben Dorman indicated  by BD 7/99
   
   $Log: udmget.c,v $
   Revision 1.18  2014/04/21 21:05:36  irby
   Rework incorrectly formed ifdef in previous revision.

   Revision 1.17  2014/04/14 21:11:38  irby
   Rework 64-bit ifdefs, adding Cygwin and dropping alpha.

   Revision 1.16  2014/02/11 17:30:36  irby
   Add block to udm_malloc that allows mmap to work on Mac OS X 64-bit
   architecture.  Differs from the existing mmap call in that the Macs do
   not define MAP_32BIT (see /usr/include/bits/mman.h), so we should also
   check that ptr does not exceed 2^32 (still to do).
...
#if (defined(__x86_64__) && (defined(__GLIBC__) || defined(__APPLE__) || defined(__CYGWIN__)))
#include <sys/mman.h>
#endif
...
static void* udm_malloc(size_t size)
{
  ...
  /* works by using mmap to allocate memory in an anonymous map,
...
#if (defined(__APPLE__) || defined(__CYGWIN__)) /* No MAP_32BIT on Mac OS X or Cygwin: */
  ptr = mmap(NULL, size + sizeof(size_t), PROT_READ | PROT_WRITE,
	     MAP_PRIVATE | MAP_ANON,
	     -1, 0);
#else /* MAP_32BIT is available on Linux: */
  ptr = mmap(NULL, size + sizeof(size_t), PROT_READ | PROT_WRITE,
...
static void* udm_realloc(void* ptr, size_t size)
{
  ...
  if( ptr != NULL && newptr != NULL )
    {
      size_t oldsize = *(size_t*)(ptr - sizeof(size_t)) - sizeof(size_t);
