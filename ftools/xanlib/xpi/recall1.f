**==recall1.spg  processed by SPAG 4.50F  at 15:17 on 26 Aug 1994
      SUBROUTINE RECALL1(N_max,Start_no,Command_no,Flag,Filename,Ierr)
...
c
     ...
     &          total
      ...
      IF ( Flag.LT.1 .OR. Flag.GT.3 ) THEN
         WRITE (ZWRite,
     &          '('' subroutine recall1 called with wrong flag '')')
...
c
         IF ( Ierr.NE.0 ) THEN
            WRITE (ZWRite,99001) Ierr , Filename(:len(ZWRite)-30)
            CALL XWRITE(ZWRite,5)
            RETURN
         ENDIF
c
