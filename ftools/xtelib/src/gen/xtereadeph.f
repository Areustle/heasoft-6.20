      SUBROUTINE xtereadeph(lue,Jdutc,Frc,Rce,Rcs,Etut,Vce,ierr)
...
CU  update : eal HSTX         date     FEB-1994 To read from FITS ephemeris 
C                                               file
CU  update : Mike Stark       date: 05-FEB-1996 Eliminate UTC->TDT correction.
C                                               XTE time is TDT already.
c   update : Brian K Elza     date: 17-APR-1996 Fixed bug left over from
c                                               ROSAT version, and notified
c                                               proper ROSAT people.
C   status: Tested
C
...
C----------------------------------------------------------------------
      ...
      dt2 = dt*dt
      dt3 = dt2*dt
      Etut = tdbtdt(nset) + dt*tdbdot(nset)
      DO i = 1 , 3
         Rcs(i) = sun(i,nset)
         Rce(i) = earth(i,1,nset) + dt*earth(i,2,nset)
     &            + 0.5D0*dt2*earth(i,3,nset) + (1.D0/6.D0)
     &            *dt3*earth(i,4,nset)
         Vce(i) = (earth(i,2,nset) + dt * earth(i,3,nset) +
     X             DT2 / 2.0D0 * Earth(I, 4, NSet)) * DAYSEC
      ENDDO
