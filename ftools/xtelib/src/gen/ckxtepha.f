C -------------------------------------------------------------------------
*+ CK_XTEPHA
       subroutine CK_XTEPHA(chatter, phafil, iunit, ireadwrite,
     &   telescop, instrume, detnam, filter, obs_date,
     &   obs_tim, ierr)

      IMPLICIT NONE
      integer chatter, ierr, iunit,ireadwrite
      character*(*) phafil
...
C Description
C   Subroutine to perform an initial check out the input XTE PHA file
C     presented to xpcaarf.  If an acceptable extension is found
C     then a bunch of observation-related keywords are read from
c     its header and placed in the COMMON BLOCK /pha_block/.
...
C     Look for the SPECTRUM extension in the PHA file 
      ninstr = 1
      instr(1) = 'SPECTRUM'
      nsearch = maxextn
      call mvext(ireadwrite, phafil, iunit, ninstr, instr, nsearch,
     &     next, outhdu, extnam, outver,
     &     extn, errflg, chatter)

      if (errflg .ne. 0) then
         mssg = errstr//' Cannot find SPECTRUM extension'
         go to 482
      endif
