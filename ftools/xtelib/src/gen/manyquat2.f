...
C     assumes time sampling (i.e. bin size) of light curve is greater
...
C                     start and stop times and gaps in attitude file
...
C
C USEAGE:      
C      CALL manyquat2(iunit2, coor, x_lo, x_hi, x_units, y_lo, y_hi,
C     $     y_units, nquats, qtime, quats, npts, time, rate, err,
C     $     collrate, collerr, collresp, chat, ierr)
C      
C ARGUMENTS:
...
C     bin       - bin size in the light curve
...
C *******************************************************************************
      ...
      SUBROUTINE manyquat2(iunit2, coor, x_lo, x_hi, x_units, y_lo,
     $     y_hi, y_units, nquats, qtime, quats, npts, bin, tstart,
     $     tstop, time, rate, err, collrate, collerr, collresp, chat,
     $     ierr)
...
C start with the declarations
      ...
      double precision qtime(nquats), time(npts), bin
      ...
      real quat(4), att(3,3), coor(3), sccoor(3), sumquat(4)
      integer ien, icx, icy
      integer i, j, k, n, m, ftstatus
      double precision halfbin
...
c Initalization
      wrnstr = '** MANYQUAT2 WARNING:'
      errstr2 = '** MANYQUAT2 ERROR:'
...
C     how the interpolation is set up.
         do while (qtime(j+1) .lt. time(i) - halfbin)
            j = j + 1
         end do

C Get average quaternion values for time(i)
         n = 0
         do k = 1,4
            sumquat(k) = 0.
         end do

         do while (qtime(j) .lt. time(i) + halfbin .and. j .le. nquats)
            j = j + 1
            n = n + 1
            do k = 1,4
               sumquat(k) = sumquat(k) + quats(j,k)
            end do
         end do
         if (n .eq. 0) then
            write(message,'(a,a,f16.6)') wrnstr,' no quats for ',
     $           time(i)
            call fcecho(message)
            call fcecho('... Inserting an intensity of INDEF')
            collrate(i) = -99.0
            collerr(i) = -99.0
            j = j + 1
            go to 900
...
C Divide the input intensity by the coll. resp. (which is < 1)
...
C Now fill in the rest of light curve beyond tstop with indef values
      do while (i .le. npts)
         collrate(i) = -99.0
         collerr(i) = -99.0
         i = i + 1
