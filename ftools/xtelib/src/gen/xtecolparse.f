...
C
C CALLING SEQUENCE: 
C       call xtecolparse (subset, ndims, dimmax, ranges, startrange,
...
C
C****************************************************************************** 
      subroutine xtecolparse (subset, ndims, dimmax, ranges, startrange,
...
C  Find the next semi-colon
 10   semipos = index(subset(firstchar:), ';') + firstchar - 1
      ...
      if (dim .gt. ndims) then
         context = 'XTECOLPARSE'//version//': Too many dimensions '
     &        // 'requested'
         call fcerr (context)
         status = 12
         goto 999
...
C check for too many ranges
      if (ranges(dim) .gt. maxranges) then
         context = 'XTECOLPARSE'//version//': Too many ranges requested'
     &        // subset
         call fcerr (context)
         status = 11
         goto 999
...
C check for reasonable input
      if (startrange(ranges(dim), dim) .lt. 0) 
     &     startrange(ranges(dim), dim) = 0
...
C loop back for next dimension
      ...
      if (dim .ne. ndims) then
         context = 'XTECOLPARSE'//version//': Incorrect number of '
 ...
 900  status = 10
      context = 'XTECOLPARSE'//version//': Error parsing requested '
     &     //'subset '//subset
      call fcerr (context)
