...
C** First get the name of the input file
  100 CALL GTBUF('Input .DHF file, [output .SHF file]?', IER)
      IF (IER.LT.0) THEN
         GOTO 8000
      ENDIF
      CALL GTCHAR(CIN, LIN)
...
C** first argument
     ...
     &                COUT(:LENACT(COUT))
      ...
      IPRTOP(1) = 0
      INXTOP(1) = 0
      IF(CTOPIC(1:4).EQ.'HELP' .OR. CTOPIC(1:4).EQ.'help' .OR.
     :      ICHAR(CLINE(1:1)).EQ.0) THEN
         LNAM=LENACT(CNAM)
         WRITE (2, 11) CNAM(:LNAM)
      ELSE
         WRITE (2, 11) CTOPIC(:NCHAR)
      END IF
      IADD = NCHAR + 1 + ihlen
      JPRTOP = 1
...
C** Now create the string of names used to identify the equivalenced text
  ...
  260    IF (JTOP.GT.1) THEN
         ...
         WRITE (2, 21) CSTARS(:IDEPTH), CLINE(LENN-NCHAR:)
...
C** just write out the topic header and then go on
         WRITE(2,21) CSTARS(:IDEPTH), CTOPIC(ICHSTR(NTOP):ICHSTP(NTOP))
         GOTO 200
      ENDIF
