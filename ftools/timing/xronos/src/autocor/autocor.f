      ...
      parameter (subname = 'autocr:')
      include '../../include/xronos.inc'
      integer i,j,iend,nk, idum
...
C*************************************************
      ...
      call xrversion('autocor', taskname)
      iend=0
      status = 0
      yi = 0
      progname = 'ac'
...
C     reallocate yi if "fast" routine
      if(iflags(15).eq.1) then
         call udmfre(yi,6,status)
         call udmget(2*nbint,6,yi,status)
      endif
...
C     work arrays yp and syp are nanal for this tool
     ...
     $        get_intsta(memi(intsta),iflags(10),nper,9,1,1)
      ...
      call xracsf(ipf, iend, memi(intsta), memr(rntsta), memd(dntsta),
     ...
     $     iframe, status)
      if(status.ne.0) then
         errm = subname//' '//'Trouble with autocorrelation routine'
         call xaerror(errm, 5)
         goto 999
      endif
