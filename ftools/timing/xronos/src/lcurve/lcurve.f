...
C          task common
      ...
      call xrversion('lcurve', taskname)
      iend=0
      status = 0
      progname = 'lc'
      progtype = 'TIME'
...
C     Allocate accumulation arrays
     ...
     $        get_intsta(memi(intsta),iflags(10),nper,9,1,1)
      ...
      call xrlc(nanal,iflags(10), xnorm, 1.0,
     $     memr(yi),memr(syi),memr(expi),memr(yr),memr(syr),memr(expr))
C     Free arrays
...
C     Allocate results x-axis
     ...
     $     iflags, rflags, memi(intsta), memr(rntsta), memd(dntsta), 
