*+WTRMF5
      SUBROUTINE WTRMF5(Ounit,Chatter,Extname,Extname_comm,
     &                  Nk_hist,Hist,Nk_comm,Comment,
...
c zpan (1.5.0 Jan 10, 2003)
c          . read/write extname from input file to output file
c --------------------------------------------------------------------------
      character(7) VERSION
      PARAMETER (VERSION='1.5.0')
*-
c Internals
      character(6) SUBNAME
      PARAMETER (SUBNAME='wtrmf5')
...
c WRITE THE EXTNAME keyword
      CALL FTUKYS(Ounit,'EXTNAME',Extname,Extname_comm,status) 
      IF ( status.NE.0 ) THEN
         CALL WTFERR(SUBNAME,VERSION,status,
