*+WTEAR1
	subroutine wtear1(ounit, chatter, 
     &		nk_hist, hist, nk_comm, comment, earversn,
	...
	integer icoord1, icoord2, ienerg, fcstln
	integer max_en, max_ic1, max_ic2
	real energ_lo(*), energ_hi(*)
	real ear(max_en,max_ic1,max_ic2)
	real coord1(*), coord2(*)
	character*(*) coordsys, c1units, c2units, enunits, earunits
	character*(*) telescop, instrume, detnam, filter, earversn
	character*(*)hist(nk_hist), comment(nk_comm)
c 
c Description:
c  Creates and writes an Effective Area extension conforming to 
...
c  	!!! Note !!!! 
c  If ICOORDN=1 and COORDN(1)=-999, then routine will assume ear not a 
...
c  EARVERSN      i   : String denoting OGIP HDUVERS2 family
...
c  Ian M George     (94 Jul 31:1.1.1), took out hduclas3 as passed parameter
	character(7) version
...
c Initialize
	ierr = 0
	status = 0
        errstr = '** WTEAR1 '//version//' ERROR: '
        wrnstr = '** WTEAR1 '//version//' WARNING: '
	qen = .true.
	qth = .true.
	qph = .true.
...
c Check we can deal with the format
     ...
     &          ' of formats'
          call fcecho(message)
          message = ' ...... requested format: '// earversn
          call fcecho(message)
          ierr = 15
          goto 998
        endif
c Check we know the format
        if(earversn.EQ.'1.1.0') then
                hduvers2 = earversn
        else
                hduvers2 = '1.1.0'
                message = wrnstr // 'Unknown format: '//earversn
                call fcecho(message)
                message =
...
c Set up the columns n stuff
	...
	tform(kk)   = wrtstr(1:10)
	tunits(kk)  = earunits
	tcomm(kk)   = 'Effective Area dataset'

c Write the required header keywords
	call FTPHBN(ounit,1,ncols,ttype,tform,tunits,
     &		'EFFECTIVE AREA',0,status)
	message = errstr
	call wt_ferrmsg(status, message)
        if(status.NE.0) then
		ierr = 1
...
c The vector coordinate keywords
	...
	if(qen)then
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		'ENERGY',
     &   	'Axis of 1st dimension of EFFAREA array',
     &		status)
	endif
	if(qth) then
	 jj = jj + 1
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		cnm1,
     &   	'Axis of 2nd dimension of EFFAREA array',
     &		status)
	endif
	if(qph) then
	 jj = jj + 1
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		cnm2,
     &   	'Axis of 3rd dimension of EFFAREA array',
     &		status)
	endif
