*+WTSPR1
	subroutine wtspr1(ounit, chatter, 
     &		nk_hist, hist, nk_comm, comment, sprversn, 
	...
	real spec_resp(max_en,max_ic1,max_ic2)
	real coord1(*), coord2(*)
	character*(*) coordsys, c1units, c2units, enunits, sprunits
	character*(*) telescop, instrume, detnam, filter, sprversn
	character*(*)hist(nk_hist), comment(nk_comm)
c 
c Description:
c  Creates and writes a Spectral Responce extension conforming to 
c   the HDUVERS2 = '1.*.*' family.
c  Currently the following formats are supported (see CAL/GEN/92-027)
c   HDUVERS2 = '1.1.0'
...
c  	!!! Note !!!! 
c  If ICOORDN=1 and COORDN(1)=-999, then routine will assume spec_resp not a 
...
c  SPRVERSN      i   : String denoting OGIP HDUVERS2 family
...
c  SPEC_RESP     i   : Array containing the full Spec_resp matrix (cm**2)
c  SPRUNITS      i   : String giving physical units of Spec_resp array
...
c  Ian M George     (93 Jan 04:0.8.0), original test version
c  Ian M George     (93 Mar 29:0.9.0), converted to use fcecho etc
c  Ian M George     (94 Jan 12:1.0.0), added HDU stuff
c  Ian M George     (94 Jul 25:1.1.0), added -999 handling
	character(7) version
...
c Initialize
	ierr = 0
	status = 0
        errstr = '** WTSPR1 '//version//' ERROR: '
        wrnstr = '** WTSPR1 '//version//' WARNING: '
	qen = .true.
	qth = .true.
	qph = .true.
...
c Check we can deal with the format
     ...
     &          ' of formats'
          call fcecho(message)
          message = ' ...... requested format: '// sprversn
          call fcecho(message)
          ierr = 15
          goto 998
        endif
c Check we know the format
        if(sprversn.EQ.'1.1.0') then
                hduvers2 = sprversn
        else
                hduvers2 = '1.1.0'
                message = wrnstr // 'Unknown format: '//sprversn
                call fcecho(message)
                message =
...
c Check for sillies
	if(ncols.EQ.1) then
	 	message = wrnstr // 
     &		'SPEC_RESP dataset contains only a single value'
	 	call fcecho(message)
	endif
...
c Set up the columns n stuff
	...
	tform(kk)   = wrtstr(1:10)
	tunits(kk)  = sprunits
	tcomm(kk)   = 'Spectral Response dataset'

c Write the required header keywords
	call FTPHBN(ounit,1,ncols,ttype,tform,tunits,
     &		'SPECTRAL RESPONSE',0,status)
	message = errstr
	call wt_ferrmsg(status, message)
        if(status.NE.0) then
		ierr = 1
...
c
	...
	call FTPKYS(ounit,'HDUCLAS2 ',
     &		'SPECRESP',
     & 		'dataset is a spectral response',
     &		status)
	...
	call FTPKYS(ounit,'HDUVERS2 ',
     &		hduvers2,
     &          'Version of format (OGIP memo CAL/GEN/92-027)',
     &		status)
...
c The TDIM (matrix ordering) keyword
	...
	call FTPKYS(ounit, wrtstr2,
     &		wrtstr,
     &   	'Ordering of n-d SPECRESP array',
     &		status)
...
c The vector coordinate keywords
	...
	if(qen)then
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		'ENERGY',
     &   	'Axis of 1st dimension of SPECRESP array',
     &		status)
	endif
	if(qth) then
	 jj = jj + 1
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		cnm1,
     &   	'Axis of 2nd dimension of SPECRESP array',
     &		status)
	endif
	if(qph) then
	 jj = jj + 1
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		cnm2,
     &   	'Axis of 3rd dimension of SPECRESP array',
     &		status)
	endif
...
c		... the spectral response dataset
...
998	if(ierr.NE.0) then
     ...
     :			' FATAL: Extension not successfully written'
	  ...
	  if(chatter.GE.20) then
		message = ' SPECRESP extension successfully written'
	   	call fcecho(message)
	  endif
	endif
