*+WTTRS1
	subroutine wttrs1(ounit, chatter, 
     &		nk_hist, hist, nk_comm, comment, trsversn, hduclas3,
	...
	real trans(max_en,max_ic1,max_ic2)
	real energ_lo(*), energ_hi(*)
	real coord1(*), coord2(*)
	character*(*) coordsys, c1units, c2units, enunits
	character*(*) telescop, instrume, detnam, filter, trsversn
	character*(*)hist(nk_hist), comment(nk_comm), hduclas3
c 
c Description:
c  Creates and writes a Transmission extension conforming to 
...
c  	!!! Note !!!! 
c  If ICOORDN=1 and COORDN(1)=-999, then routine will assume transmission not a 
...
c  TRSVERSN      i   : String denoting OGIP HDUVERS2 family
...
c  TRANS         i   : Array containing the full transmission matrix
...
c Initialize
	ierr = 0
	status = 0
        errstr = '** WTTRS1 '//version//' ERROR: '
        wrnstr = '** WTTRS1 '//version//' WARNING: '
	qen = .true.
	qth = .true.
	qph = .true.
...
c Check we can deal with the format
     ...
     &          ' of formats'
          call fcecho(message)
          message = ' ...... requested format: '// trsversn
          call fcecho(message)
          ierr = 15
          goto 998
        endif
c Check we know the format
        if(trsversn.EQ.'1.1.0') then
                hduvers2 = trsversn
        else
                hduvers2 = '1.1.0'
                message = wrnstr // 'Unknown format: '//trsversn
                call fcecho(message)
                message =
...
c Check for sillies
	if(ncols.EQ.1) then
	 	message = wrnstr // 
     &		'Trans dataset contains only a single value'
	 	call fcecho(message)
	endif
...
c
	...
	call FTPKYS(ounit,'HDUVERS2 ',
     &		hduvers2,
     &          'Version of format (OGIP memo CAL/GEN/92-024)',
     &		status)
	message = wrnstr // ' Problem putting HDUVERS2 keyword '
	call wt_ferrmsg(status, message)
	status = 0

        if(hduclas3.EQ.'WINDOW') then
                string = 'for device given by INSTRUME (+DETNAM) kywrd'
        elseif(hduclas3.EQ.'FILTER') then
                string = 'for device given by FILTER keyword'
        else
...
c The TDIM (matrix ordering) keyword
	...
	call FTPKYS(ounit, wrtstr2,
     &		wrtstr,
     &   	'Ordering of n-d TRANSMIS array',
     &		status)
...
c The vector coordinate keywords
	...
	if(qen)then
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		'ENERGY',
     &   	'Axis of 1st dimension of TRANSMIS array',
     &		status)
	endif
	if(qth) then
	 jj = jj + 1
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		cnm1,
     &   	'Axis of 2nd dimension of TRANSMIS array',
     &		status)
	endif
	if(qph) then
	 jj = jj + 1
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		cnm2,
     &   	'Axis of 3rd dimension of TRANSMIS array',
     &		status)
	endif
...
998	if(ierr.NE.0) then
     ...
     :			' FATAL: Extension not successfully written'
	  ...
	  if(chatter.GE.20) then
		message = ' Transmission extension successfully written'
	   	call fcecho(message)
	  endif
	endif
