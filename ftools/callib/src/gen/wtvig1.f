*+WTVIG1
	subroutine wtvig1(ounit, chatter, 
     &		nk_hist, hist, nk_comm, comment, vigversn, hduclas3,
	...
	real vig(max_en,max_ic1,max_ic2)
	real energ_lo(*), energ_hi(*)
	real coord1(*), coord2(*)
	character*(*) coordsys, c1units, c2units, enunits
	character*(*) telescop, instrume, detnam, filter, vigversn
	character*(*)hist(nk_hist), comment(nk_comm), hduclas3
c 
c Description:
c  Creates and writes a Vignetting extension conforming to 
...
c  	!!! Note !!!! 
c  If ICOORDN=1 and COORDN(1)=-999, then routine will assume VIG not a 
...
c  VIGVERSN      i   : String denoting OGIP HDUVERS2 family
...
c  VIG           i   : Array containing the full vignetting matrix 
...
c  Ian M George     (94 Jan 12:1.0.0), orignal
	character(7) version
...
c Initialize
	ierr = 0
	status = 0
        errstr = '** WTVIG1 '//version//' ERROR: '
        wrnstr = '** WTVIG1 '//version//' WARNING: '
	qen = .true.
	qth = .true.
	qph = .true.
...
c Check we can deal with the format
     ...
     &          ' of formats'
          call fcecho(message)
          message = ' ...... requested format: '// vigversn
          call fcecho(message)
          ierr = 15
          goto 998
        endif
c Check we know the format
        if(vigversn.EQ.'1.1.0') then
                hduvers2 = vigversn
        else
                hduvers2 = '1.1.0'
                message = wrnstr // 'Unknown format: '//vigversn
                call fcecho(message)
                message =
...
c Check for sillies
	if(ncols.EQ.1) then
	 	message = wrnstr // 
     &		'VIGNET dataset contains only a single value'
	 	call fcecho(message)
	endif
...
c
	...
	call FTPKYS(ounit,'HDUVERS2 ',
     &		hduvers2,
     &          'Version of format (OGIP memo CAL/GEN/92-021)',
     &		status)
	message = wrnstr // ' Problem putting HDUVERS2 keyword '
	call wt_ferrmsg(status, message)
	status = 0

      if(hduclas3.EQ.'TELESCOPE') then
                string = 'Vignetting (excluding any obscuration)'
        elseif(hduclas3.EQ.'TOTAL') then
                string = '(includes both vignetting & obscuration)'
        else
...
c The vector coordinate keywords
	...
	if(qen)then
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		'ENERGY',
     &   	'Axis of 1st dimension of VIGNET array',
     &		status)
	endif
	if(qth) then
	 jj = jj + 1
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		cnm1,
     &   	'Axis of 2nd dimension of VIGNET array',
     &		status)
	endif
	if(qph) then
	 jj = jj + 1
	 ...
	 call FTPKYS(ounit, wrtstr2,
     &		cnm2,
     &   	'Axis of 3rd dimension of VIGNET array',
     &		status)
	endif
...
998	if(ierr.NE.0) then
     ...
     :			' FATAL: Extension not successfully written'
	  ...
	  if(chatter.GE.20) then
		message = ' VIGNET extension successfully written'
	   	call fcecho(message)
	  endif
	endif
