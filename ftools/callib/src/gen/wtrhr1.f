...
c --- DESCRIPTION -------------------------------------------------
c
c This subroutine writes a ROSAT HRI DETECTOR extension for a PHA file in 
...
c The 1992a format for the ROSAT HRI consists of a BINTABLE extension, with
...
c Initialization
	...
	status = 0
	errstr = '** WTRHR1 ERROR :'
	wrnstr = '** WTRHR1 WARNING :'

c --- USER INFO ---
c
      IF (chatter.GE.20) THEN
         subinfo = ' ... using WTRHR1 Ver '//version
         call fcecho(subinfo)
      ENDIF
...
c Add the (passed) history cards, adding one related to this programme
	itemp = 0
     ...
     &           status)
        ...
        enddo
        write(subinfo,'(2a)')
     &		' HRI DETECTOR extension written by WTRHR1 ',
     &                          version
...
c ... reassure if everything OK
	...
	if(chatter.GE.5) then
	   subinfo = ' ... written the ROSAT HRI DETECTOR extension'
	   call fcecho(subinfo)
	endif
