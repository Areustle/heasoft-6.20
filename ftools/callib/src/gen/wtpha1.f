...
c   QUALITY or GROUPING columns, then that column is not written and the 
...
c   Rehana Yusaf (1.0.0: 1993 Apr 07), original based on KAAs routine
c   Ian M George (2.0.0: 1993 Oct 12), renamed from wt_pha1992a & major 
c					overhaul of HDUCLAS/VERS stuff
c   Ian M George (3.0.0: 1993 Oct 22), added the fchan passed parameter
c   Ian M George (3.0.1: 1994 Feb 16), made passed parameters*(*)
c   Ian M George (3.0.2: 1994 Apr 01), set corrscal/backscal = 0 when
c						if corrfil/backfil = 'NONE'
c   Ian M George (3.0.3: 1994 Apr 21), fixed units str on STAT_ERR when dtype=1
c   Ian M George (3.0.4: 1994 May 10), made COUNT col I*4 (rather than I*2)
c   Ian M George (3.0.5: 1994 Jul 21), removed 1994 Apr 01 change
c   Ian M George (3.0.6: 1994 Aug 01), allowed hduclas2 = UNKNOWN
c   Ian M George (3.1.0: 1994 Dec 14), POISERR = F when qerror = T
c   Ian M George (3.1.1: 1994 Dec 15), POISERR = F when false
c   Rehana Yusaf (3.1.2: 1995 Jan 19), column name 'CHANNEL' not ' CHANNEL'
c   Ian M George (3.1.3: 1995 Jun 22), maxsize increased from 2048 to 4096
c 
c Banashree Mitra Seifert (3.2.0: June 1996) maxsize increased from 4096 
c                                            to 1000000 
c   Ian M George (3.2.1: 1995 Sep 12), made conv4=NINT(counts)
c   Ziqin Pan (Oct. 5 2001) 3.2.2
c         . Add codes to handle undfined value for "Counts"
c   Bryan Irby (Mar 28 2003) 3.2.3:
...
c    if applicable.  Fix null values.
c
c ----------------------------------------------------------------------
      character(5) version
      parameter (version = '3.2.3')
*-
c Internals 
	integer maxsize
	...
	character(30) errstr, wrnstr
        ...
        integer tnull(nfields)
	integer inull
        real rnull
        integer strlen, fcstln

c Initialization
	ierr = 0
	status = 0
	errstr = '** WTPHA1 '//version//' ERROR:'
	wrnstr = '** WTPHA1 '//version//' WARNING:'
        do i = 1, nfields
          tnull(i) = 0
        enddo
...
c
      IF (chatter.GE.20) THEN
         subinfo = ' ... using WTPHA1 Ver '//version
         call fcecho(subinfo)
      ENDIF
c Check for sillies
...
c Setup header keywords
	...
	elseif(chantyp.eq.'PI' .OR. chantyp.EQ.'pi') then
      		tcomm(1) = 'Pulse Invariant (PI) Channel '
	else
	  ...
	  if(status.NE.0) then
	   ...
	   status = 0
          ...
          if((i.NE.3).and.(i.NE.4)) then
           if(.NOT.((i.EQ.2).and.(dtype.NE.1))) then
            write(keywrd,'(a5,i2)') 'TNULL',i
            call crmvblk(keywrd)
            call ftpkyj(ounit,keywrd,tnull(i),'Null value',status)
           endif
          endif
	  if(status.NE.0) then
	   subinfo = wrnstr // 'Problem altering '// keywrd
...
c Add the (passed) history cards, adding one related to this programme
	itemp = 0
     ...
     &           status)
        ...
        enddo
        write(subinfo,'(2a)')
     &		' FITS SPECTRUM extension written by WTPHA1 ',
     &                          version
...
c --- WRITE COUNTS ---
      ...
      IF (dtype .EQ. 1) THEN
         ...
         enddo
c         CALL ftpclj(ounit,colnum,frow,felem,nchan,conv4,status)
       CALL ftpcnj(ounit,colnum,frow,felem,nchan,conv4,inull,status)
      ELSEIF (dtype .EQ. 2) THEN
c         CALL ftpcle(ounit,colnum,frow,felem,nchan,counts,status)
       CALL ftpcne(ounit,colnum,frow,felem,nchan,counts,rnull,status)
      ENDIF
      	if(status.NE.0) then
