*+WTCOL1
	subroutine wtcol1(ounit, chatter, 
     &		nk_hist, hist, nk_comm, comment, crspvers, hduclas3,
	...
	real collresp(max_en,max_ic1,max_ic2)
	real energ_lo(*), energ_hi(*)
	real coord1(*), coord2(*)
	character*(*) coordsys, c1units, c2units, enunits, crunits
	character*(*) telescop, instrume, detnam, filter, crspvers
	character*(*)hist(nk_hist), comment(nk_comm), hduclas3
c 
c Description:
c  Creates and writes a Collimator response extension conforming to 
c   the HDUVERS2 = '1.*.*' family.
c  Currently the following formats are supported (see CAL/GEN/92-???)
c   HDUVERS2 = '1.1.0'
...
c  	!!! Note !!!! 
c  If ICOORDN=1 and COORDN(1)=-999, then routine will assume coll response 
c	not a function of that spatial variable.
...
c  CRSPVERS      i   : String denoting OGIP HDUVERS2 family
...
c  COLLRESP      i   : Array containing the full Coll resp matrix (cm**2)
c  CRUNITS       i   : String giving physical units of COLLRESP array
...
c  subroutine CRMVBLK    : (CALLIB) Removes blank from a string
c  subroutine CRMVLBK    : (CALLIB) Removes leading blanks from a string
c  subroutine FCECHO     : (FITSIO) Writes to STDOUT
...
c  subroutine FTMCOM	 : (FITSIO) Modifies a comment
...
c  subroutine GCSYNM     : (CALLIB) Returns col names for coord system
...
c  Ian M George     (94 Jul 28:0.9.0), original
c  Ian M George     (94 Oct 24:1.0.0), additional error checks 
c  Ian M George     (95 May 24:1.0.1), CREF keyword now written
c  Ian M George     (95 Jul 08:1.0.2), Revised axis-type syntax
c  Ian M George     (95 Jul 08:1.0.3), replaced GT_CSYSNMS w/ GCSYNM
	character(7) version
...
c Initialize
	ierr = 0
	status = 0
        errstr = '** WTCOL1 '//version//' ERROR: '
        wrnstr = '** WTCOL1 '//version//' WARNING: '
	qen = .true.
	qth = .true.
	qph = .true.
...
c Check for sillies
	if(ienerg.LE.0) then
	   message = errstr // ' Too few energy bins'
	   call fcecho(message)
	   write(message,'(a,i12)') 
     &		' ...... No. bins = ', ienerg
	   call fcecho(message)
	...
	if(icoord1.LE.0) then
	   message = errstr // 
     &		' Too few spatial bins in coord-1'
	   call fcecho(message)
	   write(message,'(a,i12)') 
     &		' ...... No. bins = ', icoord1
	   call fcecho(message)
	...
	if(icoord2.LE.0) then
	   message = errstr // 
     &		' Too few spatial bins in coord-2'
	   call fcecho(message)
	   write(message,'(a,i12)') 
     &		' ...... No. bins = ', icoord2
	   call fcecho(message)
...
c Check we can deal with the format
     ...
     &          ' of formats'
          call fcecho(message)
          message = ' ...... requested format: '// crspvers
          call fcecho(message)
          ierr = 15
          goto 998
        endif
c Check we know the format
        if(crspvers.EQ.'1.1.0') then
                hduvers2 = crspvers
        else
                hduvers2 = '1.1.0'
                message = wrnstr // 'Unknown format: '//crspvers
                call fcecho(message)
                message =
...
c Sort out the coordinate system & column names
        call gcsynm(chatter, 0, coordsys,
     &          cnm1, cnm2, dummy8, dummy8, ierr)
        ierr = 0
        status = 0
...
c Check for sillies
	if(ncols.EQ.1) then
	 	message = wrnstr // 
     &		'COLLRESP dataset contains only a single value'
	 	call fcecho(message)
	endif
...
c Set up the columns n stuff
	...
	tform(kk)   = wrtstr(1:10)
	tunits(kk)  = crunits
	tcomm(kk)   = 'Collimator Response dataset'

c Write the required header keywords
	call FTPHBN(ounit,1,ncols,ttype,tform,tunits,
     &		'COLLIMATOR RESPONSE',0,status)
	message = errstr
	call wt_ferrmsg(status, message)
        if(status.NE.0) then
		ierr = 1
...
c
	...
	call FTPKYS(ounit,'HDUCLAS2 ',
     &		'COLLRESP',
     & 		'dataset is a collimator response',
     &		status)
	...
	call FTPKYS(ounit,'HDUVERS2 ',
     &		hduvers2,
     &          'Version of format (OGIP memo CAL/GEN/92-019)',
     &		status)
	message = wrnstr // ' Problem putting HDUVERS2 keyword '
	call wt_ferrmsg(status, message)
	status = 0

      if(hduclas3.EQ.'SCATTERING') then
             string = 'dataset is a Collimator Scattering Correction'
        elseif(hduclas3.EQ.'TRANSMISSION') then
             string = 'dataset is a Collimator Transmission Correction'
        elseif(hduclas3.EQ.'TOTAL') then
             string = '(includes any scattering, transmission effects)'
        else
...
c Add other passed keyword values
	...
	if(.not.qen) then
	  wrtstr = 'Dataset is independent of ENERGY'
...
c The TDIM (matrix ordering) keyword
	...
	call FTPKYS(ounit, wrtstr2,
     &		wrtstr,
     &   	'Ordering of n-d COLLRESP array',
     &		status)

c The CREF (Column Referencing) keyword
	status = 0
	write(wrtstr2, '(a,i12)') 'CREF', ncols
        call crmvblk(wrtstr2)

	wrtstr = '('
	if(qen) then
		wrtstr = wrtstr(:clenact(wrtstr))//'ENERG_LO:ENERG_HI'
	endif
	if(qth) then
	     if(qen) then
		wrtstr = wrtstr(:clenact(wrtstr))//','//cnm1
       	     else
		wrtstr = wrtstr(:clenact(wrtstr))//cnm1
	     endif	
	endif
	if(qph) then
	     if(qth) then
		wrtstr = wrtstr(:clenact(wrtstr))//','//cnm2
	     else
		wrtstr = wrtstr(:clenact(wrtstr))//cnm2
	     endif	
	endif
	...
	call FTPKYS(ounit, wrtstr2,
     &		wrtstr,
     &   	'Column referencing of n-d COLLRESP array',
     &		status)
...
998	if(ierr.NE.0) then
     ...
     :			' FATAL: Extension not successfully written'
	  ...
	  if(chatter.GE.20) then
		message = ' COLLRESP extension successfully written'
	   	call fcecho(message)
	  endif
	endif
