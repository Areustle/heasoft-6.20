CXIM - Check for ITF = 10 (Assign colors to levels)
...
CXIM
C*XGRIMG2 -- image of a 2D data array (pixel-primitive devices)
C+
      SUBROUTINE XGRIMG2 (A, IDIM, JDIM, I1, I2, J1, J2,
...
C-----------------------------------------------------------------------
      INCLUDE 'grpckg1.inc'
c
c  XIMAGE mods (levels and null capability)
c
      include 'level.inc'
...
c
c  Special ximage mode
c
                if ( isrnull(AV) ) then
                   IV = NULlcol
                else
                   IV = MININD + location(levels,numlevs,AV)
                endif
            ELSE
                IV = MININD
            END IF
