...
#=========================================================================#
# Differs from browse_extract.pl in that xwebquery is used instead of
#    webquery. Version 2.1a of browse_extract march 2006
####################################################################
#
...
#
#  All argument keys are case-insensitive.
#
...
# webquery.  The program can easily be modified to use other such tools,
# e.g., wget.
#
######################################################################
#
...
Usage:
   ...
   as the name does not conflict with another browse_extract parameter.
...
sub do_position {
    ...
    my $cmd = "$webquery host=$host url=$url method=GET tablehead=$Tablehead ".
                       "Action=$Action Coordinates=$Coordinates ".
		       "Equinox=$Equinox Radius=$Radius NR=$NR ".
		       "GIFsize=$GIFsize Fields=$Fields";
    ...
    $cmd .= " sortvar=".$args{sortvar}     if $args{sortvar};
    $cmd .= " ResultMax=".$args{resultmax} if $args{resultmax};
    $cmd .= " Time=".$args{"time"} if $args{"time"};
    ...
    my $display = $stdFormats{$format};
    if (!defined($display)) {
        $display = "BatchDisplay";
    }
    
    $cmd .= " displaymode=$display";
