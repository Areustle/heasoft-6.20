...
INFILE* openInfile(char* name) {
    ...
    char col_sin_angle[FLEN_VALUE];
    char col_cos_angle[FLEN_VALUE];
    ...
    ****************/
    ...
    /*******************
    * Find the columns.  The COSANGLE and SINANGLE columns do not need
    * to exist, but the others must. *
    *******************/
    read_string_param("deltaxcol", col_x, FLEN_VALUE);
    read_string_param("deltaycol", col_y, FLEN_VALUE);
    read_string_param("sincol", col_sin_angle, FLEN_VALUE);
    read_string_param("coscol", col_cos_angle, FLEN_VALUE);
    ...
    fits_get_colnum(infile->fp, CASEINSEN, col_sin_angle, &(infile->col_sin_angle), &status);
    if(status == COL_NOT_FOUND)
      {
	infile->col_sin_angle = -1;
	status = 0;
      }
    fits_get_colnum(infile->fp, CASEINSEN, col_cos_angle, &(infile->col_cos_angle), &status);
    if(status == COL_NOT_FOUND)
      {
	infile->col_cos_angle = -1;
	status = 0;
      }
    fits_get_colnum(infile->fp, CASEINSEN, "TIME", &(infile->col_time), &status);

    if(infile->col_sin_angle == -1 || infile->col_cos_angle == -1)
      {
	infile->use_angle = 0;
	headas_chat(4, "Warning: Could not find one or both of the input file columns containing the cosine or sine of the rotation angle.\nContinuing by assuming that no time-dependent rotation is involved.\n");
      }
    else
      infile->use_angle = 1;
...
int readInfileValues(INFILE* infile,
    ...
    ******************/
    ...
    if(infile->use_angle)
      {
	fits_read_col_dbl(infile->fp, infile->col_sin_angle, infile->row,
                      1l, 1l, 0.0, sin_angle, &anynull, &status);
	if(anynull)
	  *sin_angle = 0.;

	fits_read_col_dbl(infile->fp, infile->col_cos_angle, infile->row,
                      1l, 1l, 0.0, cos_angle, &anynull, &status);
	if(anynull)
	  *cos_angle = 1.;
