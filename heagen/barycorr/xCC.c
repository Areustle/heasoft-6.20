char *xccrcsid = "RCS: $Id: xCC.c,v 1.3 2012/10/17 19:54:29 craigm Exp $" ;
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
#include <string.h>
...
int xCC (double t, double *tZero, double *tcorr, char *clockFile)
{
  char *xteClockFile;
  ...
  /* Use clock file name if requested, otherwise default to XTECC */
  xteClockFile = clockFile ? clockFile : XTECC;
  if ( !( tdc = openAFile (xteClockFile) ) ) {
    error = -1 ;
    corr = 0 ;
  }
