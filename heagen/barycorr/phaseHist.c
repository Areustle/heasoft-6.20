char *phrcsid = "RCS: $Id: phaseHist.c,v 1.2 2004/07/12 20:27:34 miket Exp $" ;
/*-----------------------------------------------------------------------
 *
 *  phaseHist.c
 *
 ...
 */
char *phProgName = {"phaseHist $Revision: 1.2 $"} ;
fitsfile *data_file;
int htype ;
char timesys[80] = "UTC" ;
char filetype[80] ;
...
/*        The other columns - by mission  */
    ...
    if ( debug )
      fprintf (stderr, "%s: read %d rows, starting at %d after %d events, error %d\n",
	       phProgName, nbufrow, row+1, nevents, error) ;
 ...
 */
	...
	phase -= (int) phase ;
	if ( debug > 7 ) {
	  fprintf (stderr, "Event %7d   Chan %5d   TB = %18.17g   Phase = %6.3g\n",
		   nevents, chan, t, phase) ;
	}
	j = phase * dnphase ;
	if ( chan >= numchan )
	  chan = numchan - 1 ;
