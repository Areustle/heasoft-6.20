char *bcrcsid = "RCS: $Id: bary.c,v 1.4 2012/10/17 19:54:29 craigm Exp $" ;
/*-----------------------------------------------------------------------
 *
 *  bary.c
 *
 ...
 */
  scorbitinit (mission) ;
  /* Note that clockFile=0 as below will cause previously initialized
     clock file name (if any) to be preserved */
  clockinit (mission, 0) ;
...
double barycorr (MJDTime *mjdXT, double *sourcedir, double *scposn)
{
  ...
  double rca[3], rsa[3];
  const double *eposn ;
  double total, sundis, sunsiz, cth, t2b ;
  MJDTime mjdXX ;
  MJDTime *mjdTT ;
  int i;
  int iearth = 3 ;
	       ...
	       PsrTimPar *ptp, PsrBinPar *pbp, int debug)
{
  ...
  double ras, decs, mjdf, fd, dfd, ddfd, dddfd, ddddfd, rms, lastrms, mjdftrun ;
  double tr, dtt, dir[3] ;
  MJDTime mtime ;
 ...
 */
  ...
  while ( strcmp (dsource, source) && !error ) {
    ...
    timeline (line, bsource, &ra, &dec, &mjd1, &mjd2, &mjdi, &mjdf,
	      &fd, &dfd, &ddfd, &dddfd, &ddddfd, &rms, &fbin, &tdenum, jsource) ;
  }
  if ( error > 1 )
 ...
 */
	...
	printf ("===> Note that no entry could be found covering the observations.\n") ;
      ...
      printf ("  MJD(TDB) = %d + %g\n", mjdi, mjdftrun) ;
      printf ("  f = %.14f  df/dt = %g  d2fdt2 = %g  d3fdt3 = %g  d4fdt4 = %g\n", fd, dfd, ddfd, dddfd, ddddfd) ;
    ...
    ptp->radioph = -fd*dtt - dfd*dtt*dtt*0.5 - ddfd*dtt*dtt*dtt/6.0
      - dddfd*dtt*dtt*dtt*dtt/24.0 - ddddfd*dtt*dtt*dtt*dtt*dtt/120.0 ;
    if ( debug )
...
timeline (char *line, char *bsource, double *ra, double *dec,
	  ...
	  double *fd, double *dfd, double *ddfd, double *dddfd, double *ddddfd,
  ...
  int dfexp, ddfexp, dddfexp, ddddfexp, nitem ;
  ...
  nitem = sscanf (line,
		  "%s %d %d %lg %s %d %lg %ld %ld %ld%lg %lg %lgD%3d%lgD%3d%lg %c %c %s %s %lgD%3d %lgD%3d",
		  bsource, &rah, &ram, &ras, &sdec, &decm, &decs,
		  mjd1, mjd2, mjdi, mjdf, fd, dfd, &dfexp, ddfd, &ddfexp, rms,
		  obs, &bino, dum1, dum2, dddfd, &dddfexp, ddddfd, &ddddfexp) ;
  obs[1] = 0 ;
		 ...
		 int bcorr, double *dir, char *oefile, int *error)
{
  ...
  if ( !(*error) ) {
    ...
    ff = ptp->f - fi ;
    phase = fi*dtf + ff*dt + ptp->df*dt2 + ptp->ddf*dt3
      + ptp->dddf*dt4 + ptp->ddddf*dt5 + ptp->radioph ;
    phase -= (int) phase ;
    if ( phase < 0.0 )
      phase++ ;
  }
