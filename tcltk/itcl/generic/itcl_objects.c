    ...
    Tcl_Obj *CONST objv[];   /* argument objects */
{
    ...
    if ( !entry ) {
       /*
        *  Command not found.  Lets check if there is a forwardInvocation
        *  command available which should handle this instead
        */
       entry = Tcl_FindHashEntry(&contextObj->classDefn->resolveCmds,
                                 "forwardInvocation");

       if( entry ) {
          /*  YES!  Redirect args to it instead  */
          int i;
          
          cmdArgc = objc+1;
          cmdArgv = (Tcl_Obj**)ckalloc( cmdArgc*sizeof(Tcl_Obj*) );
          cmdArgv[0] = objv[0];  /* Not necessary but completes Argv */
          cmdArgv[1] = Tcl_NewStringObj( "forwardInvocation", -1 );
          for(i=1;i<objc;i++) cmdArgv[i+1] = objv[i];
       }
    }
    ...
    result = Itcl_EvalArgs(interp, cmdArgc-1, cmdArgv+1);
