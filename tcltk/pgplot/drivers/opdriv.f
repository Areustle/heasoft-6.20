C*OPDRIV -- old PGPLOT PostScript drivers using vector character fonts
C+
      SUBROUTINE OPDRIV (IFUNC, RBUF, NBUF, CHR, LCHR, MODE)
...
C-----------------------------------------------------------------------
      ...
      PARAMETER (
     : PTYPE= 'OVPS   (Old PostScript file, portrait orientation)',
     : LTYPE= 'OPS    (Old PostScript file, landscape orientation)',
     : CPTYPE='OVCPS  (Old Colour PostScript file, portrait mode)',
     : CLTYPE='OCPS   (Old Colour PostScript file, landscape mode)')
...
C        -- machine-dependent!
 ...
 94         IF (CHR(LCHR:LCHR).EQ.' ') THEN
      ...
      IF (L.GT.0) CALL GROP02(IOERR, UNIT, CHAR(4))
      CALL GROP02(IOERR, UNIT, '%!PS-Adobe-3.0 EPSF-3.0')
      CALL GRUSER(INSTR, L)
      IF (L.GT.0) CALL GROP02(IOERR, UNIT, '%%For: '//INSTR(1:L))
      CALL GROP02(IOERR, UNIT, '%%Title: PGPLOT PostScript plot')
      CALL GROP02(IOERR, UNIT, '%%Creator: PGPLOT [PSDRIV 6.6]')
      CALL GRDATE(INSTR, L)
      IF (L.GT.0) CALL GROP02(IOERR, UNIT,
...
C           actual dimensions
     ...
     :        NINT((WIDTH+OFFW)*0.072), NINT((HEIGHT+OFFH)*0.072))
         CALL GROP02(IOERR, UNIT, INSTR(:L))
      ELSE
         CALL GROP02(IOERR, UNIT, '%%BoundingBox: (atend)')
      END IF
      CALL GROP02(IOERR, UNIT, '%%DocumentFonts: (atend)')
      CALL GROP02(IOERR, UNIT, '%%LanguageLevel: 1')
      LANDSC = MODE.EQ.1 .OR. MODE.EQ.3
      IF (LANDSC) THEN
          CALL GROP02(IOERR, UNIT, '%%Orientation: Landscape')
      ELSE
          CALL GROP02(IOERR, UNIT, '%%Orientation: Portrait')
      END IF
      CALL GROP02(IOERR, UNIT, '%%Pages: (atend)')
      CALL GROP02(IOERR, UNIT, '%%EndComments')
      CALL GROP02(IOERR, UNIT, '%%BeginProlog')
      CALL GROP02(IOERR, UNIT, 
     ...
     1  '/D {moveto 0 0 rlineto currentpoint stroke moveto} bind def')
      CALL GROP02(IOERR, UNIT, '/LW {5 mul setlinewidth} bind def')
      CALL GROP02(IOERR, UNIT, '/BP {newpath moveto} bind def')
      CALL GROP02(IOERR, UNIT, '/LP /rlineto load def')
      CALL GROP02(IOERR, UNIT, 
     1  '/EP {rlineto closepath eofill} bind def')
      CALL GROP02(IOERR, UNIT, '/MB {gsave translate MFAC dup scale '//
     1 '1 setlinewidth 2 setlinecap 0 setlinejoin newpath} bind def')
      CALL GROP02(IOERR, UNIT, '/ME /grestore load def')
      CALL GROP02(IOERR, UNIT, '/CC {0 360 arc stroke} bind def')
      CALL GROP02(IOERR, UNIT, '/FC {0 360 arc fill} bind def')
      CALL GROP02(IOERR, UNIT, '/G {1024 div setgray} bind def')
      CALL GROP02(IOERR, UNIT, '/K {3 -1 roll 1024 div 3 -1 roll 1024'//
     :     ' div 3 -1 roll 1024 div setrgbcolor} bind def')
      CALL GROP02(IOERR, UNIT, '% Uncomment next line to convert color'
     :     //' to grey shades')
      CALL GROP02(IOERR, UNIT, '%/K {3 -1 roll 3413 div 3 -1 roll 1739'
     ...
     :        ' stringwidth neg exch neg exch rmoveto show} bind def')
      END IF
      CALL GROP02(IOERR, UNIT, '%%EndProlog')
      CALL GROP02(IOERR, UNIT, '%%BeginSetup')
      CALL GROP02(IOERR, UNIT, '/#copies 1 def')
      CALL GROP02(IOERR, UNIT, '%%EndSetup')
      NPAGE = 0
      RETURN
C
...
C
  100 CONTINUE
      CALL GROP02(IOERR, UNIT, ' ')
      CALL GROP02(IOERR, UNIT, '%%Trailer')
      CALL GRGENV('PS_BBOX', INSTR, L)
      CALL GRTOUP(INSTR(1:3), INSTR(1:3))
     ...
     :        BBOX(1), BBOX(2), BBOX(3), BBOX(4))
         CALL GROP02(IOERR, UNIT, INSTR(:L))
      END IF
      CALL GROP02(IOERR, UNIT, '%%DocumentFonts: ')
      CALL GRFAO('%%Pages: #', L, INSTR, NPAGE, 0, 0, 0)
      CALL GROP02(IOERR, UNIT, INSTR(:L))
      CALL GROP02(IOERR, UNIT, '%%EOF')
      CALL GRGENV('PS_EOF', INSTR, L)
      IF (L.GT.0) CALL GROP02(IOERR, UNIT, CHAR(4))
      IF (IOERR.NE.0) THEN
...
C
  110 CONTINUE
      ...
      CALL GROP02(IOERR, UNIT, INSTR(:L))
      CALL GROP02(IOERR, UNIT, '%%BeginPageSetup')
      CALL GROP02(IOERR, UNIT, '/PGPLOT save def')
      CALL GROP02(IOERR, UNIT, '0.072 0.072 scale')
      LANDSC = MODE.EQ.1 .OR. MODE.EQ.3
      IF (LANDSC) THEN
     ...
     1               OFFH, 0, 0)
      ...
      END IF
      CALL GROP02(IOERR, UNIT, INSTR(:L))
      CALL GROP02(IOERR, UNIT, '1 setlinejoin 1 setlinecap 1 LW 1')
      CALL GROP02(IOERR, UNIT, '%%EndPageSetup')
      CALL GROP02(IOERR, UNIT, '%%PageBoundingBox: (atend)')
      DO 111 NSYM=0,31
          MARKER(NSYM) = 0
  111 CONTINUE
      MFAC = 0.0
...
C
  140 CONTINUE
      IF (LOBUF.NE.0) THEN
          CALL GROP02(IOERR, UNIT, OBUF(1:LOBUF))
          LOBUF = 0
      END IF
      LANDSC = MODE.EQ.1 .OR. MODE.EQ.3
C     -- optionally write identification
     ...
     :        L, INSTR, NPAGE, POSN, 50, 0)
         CALL GROP02(IOERR, UNIT, '0 G')
         CALL GROP02(IOERR, UNIT, INSTR(1:L))
      END IF
C     -- optionally draw bounding box
     ...
     :              NINT(BBXMAX), NINT(BBYMIN), 0, 0)
         CALL GROP02(IOERR, UNIT, INSTR(1:L))
      END IF
      CALL GROP02(IOERR, UNIT, 'PGPLOT restore showpage')
      CALL GROP02(IOERR, UNIT, '%%PageTrailer')
      IF (LANDSC) THEN
...
C         -- flush buffer first
          ...
          LOBUF = 0
      END IF
      CALL GROP02(IOERR, UNIT, CHR(:LCHR))
      LASTI = -1
      RETURN
C
C--- IFUNC=26, Image.---------------------------------------------------
...
C      
          CALL GROP02(IOERR, UNIT, 'gsave newpath')
...
C         -- 
          ...
          CALL GROP02(IOERR, UNIT, INSTR(:67))
          IF (COLOR) THEN
              CALL GROP02(IOERR, UNIT, 
...
C         -- Last: terminate image
          CALL GROP02(IOERR, UNIT, 'grestore')
      ELSE 
...
C     -- Output code for this marker if necessary
      IF (MARKER(NSYM).EQ.0) THEN
          IF (LOBUF.GT.0) CALL GROP02(IOERR, UNIT, OBUF(1:LOBUF))
          LOBUF = 0
          CALL GROP03(IOERR, NSYM, UNIT)
          MARKER(NSYM) = 1
      END IF
C     -- Output scale factor
      IF (RBUF(4).NE.MFAC) THEN
          ...
          CALL GROP02(IOERR, UNIT, INSTR(1:24))
      END IF
...
C*GROP03 -- PGPLOT PostScript driver, marker support
C+
      SUBROUTINE GROP03(IOERR, NSYM, UNIT)
...
C
  200 DO 210 I=1,N
          CALL GROP02(IOERR, UNIT, T(I))
  210 CONTINUE
C
      END

C*GROP02 -- PGPLOT PostScript driver, copy buffer to file
C+
      SUBROUTINE GROP02 (IER, UNIT, S)
C
